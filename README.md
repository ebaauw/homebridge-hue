...
- The built-in Daylight sensor is exposed a as a `Stateful Programmable Switch` service.  The `Output State` holds `0` (`daylight` is `false`) or `1` (`true`).  I tried exposing this sensor as a regular `Switch` using a read-only `On` characteristic, but the iOS 10 `Home` app ignores the read-only setting.  This was particularly cool under iOS 9, when Homemit didn't support rules on sunrise and sunset.  Under iOS 10 it does, even though most 3rd-party Homekit apps don't yet support this.
