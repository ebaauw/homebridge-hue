...
function closest(a, b, p) {
  ...
  var t = (ab.x * ab.x + ab.y * ab.y) / (ap.x * ab.x + ap.y * ab.y);
...
function invHueSat(hue, sat, model) {
  ...
  let p = closestInGamut({x: X / sum, y: Y / sum}, model);
...
function HueLight(bridge, id, obj, type) {
  ...
  this.log.debug("%s: set homekit power to %s", this.name, this.hk.on);
  ...
  if (this.state.bri !== undefined) {
    this.log.debug("%s: set homekit brightness to %s%%", this.name, this.hk.bri);
  ...
  if (this.state.ct !== undefined) {
    ...
    this.log.debug("%s: set homekit color temperature to %sK", this.name, this.hk.ct);
  ...
  if (this.state.xy !== undefined) {
    this.log.debug("%s: set homekit hue to %s˚", this.name, this.hk.hue);
    this.service.setCharacteristic(Characteristic.Hue, this.hk.hue);
    this.log.debug("%s: set homekit saturation to %s%%", this.name, this.hk.sat);
...
HueLight.prototype.heartbeat = function(obj) {
  ...
  if (this.state.on !== old.state.on) {
    this.log.debug("%s: %s on changed from %s to %s", this.name, this.type,
    		   old.state.on, this.state.on);
  }
  if (this.hk.on != old.hk.on) {
    this.log.info("%s: change homekit power from %s to %s", this.name,
  ...
  if (this.hk.bri != old.hk.bri) {
    this.log.info("%s: change homekit brightness from %s%% to %s%%", this.name,
  ...
  if (this.hk.ct != old.hk.ct) {
    this.log.info("%s: change homekit color temperature from %sK to %sK",
  ...
  if (this.state.xy !== undefined &&
    ...
    if (this.state.colormode === "xy") {
      ...
      this.log.debug("%s: %s xy updated by %s from %j to %j", this.name, this.type,
      		     this.state.colormode, old.state.xy, this.state.xy);
    }
  }
  if (this.hk.hue != old.hk.hue) {
    this.log.info("%s: change homekit hue from %s˚ to %s˚",
		  this.name, old.hk.hue, this.hk.hue);
    this.service.setCharacteristic(Characteristic.Hue, this.hk.hue);
  }
  if (this.hk.hue != old.hk.hue) {
    this.log.info("%s: change homekit saturation from %s%% to %s%%",
...
HueLight.prototype.identify = function(callback) {
  ...
  this.bridge.request("put", this.url, "{\"alert\":\"select\"}", function(obj) {
    return callback();
  }.bind(this));  
}
...
HueLight.prototype.setOn = function(on, callback) {
  ...
  let newOn = on ? true : false;
  this.bridge.request("put", this.url, "{\"on\":" + newOn + "}", function(obj) {
    if (!obj) {
...
HueLight.prototype.setBri = function(bri, callback) {
  ...
  this.log.info("%s: homekit brightness changed from %s%% to %s%%", this.name, this.hk.bri, bri);
  let newBri = Math.round(bri * 254 / 100);
  this.bridge.request("put", this.url, "{\"bri\":" + newBri + "}", function(obj) {
    if (!obj) {
      return callback(new Error());
    }
    this.hk.bri = bri;
...
HueLight.prototype.setCT = function(ct, callback) {
  ...
  this.log.info("%s: homekit color temperature changed from %sK to %sK", this.name, this.hk.ct, ct);
  let newCT = Math.max(153, Math.min(Math.round(1000000.0 / ct), 500));
  this.bridge.request("put", this.url, "{\"ct\":" + newCT + "}", function(obj) {
    if (!obj) {
      return callback(new Error());
    }
    this.hk.ct = ct;
...
HueLight.prototype.setHue = function(hue, callback) {
  ...
  let newXY = invHueSat(hue, this.hk.sat, this.obj.modelid);
  this.bridge.request("put", this.url, "{\"xy\":[" + newXY[0] + "," + newXY[1] + "]}", function(obj) {
    if (!obj) {
      return callback(new Error());
    }
    this.hk.hue = hue;
...
HueLight.prototype.setSat = function(sat, callback) {
  ...
  this.log.info("%s: homekit saturation changed from %s%% to %s%%", this.name, this.hk.sat, sat);
  let newXY = invHueSat(this.hk.hue, sat, this.obj.modelid);
  this.bridge.request("put", this.url, "{\"xy\":[" + newXY[0] + "," + newXY[1] + "]}", function(obj) {
    if (!obj) {
      return callback(new Error());
    }
