...
HueLight.prototype.setConfig = function () {
  ...
  this.config = {
    ...
    windowCovering: this.obj.type === 'Window covering device'
  }
  ...
  } else if (this.config.windowCovering) {
  ...
  if (this.config.xy) {
    if (
      ...
      // reported by Hue bridge
      this.config.gamut = {
        r: this.obj.capabilities.control.colorgamut[0],
        g: this.obj.capabilities.control.colorgamut[1],
        b: this.obj.capabilities.control.colorgamut[2]
      }
    } else if (this.config.unknown) {
      this.log.warn(
  ...
  for (const key in state) {
    switch (key) {
      ...
      case 'sat':
        ...
        break
      case 'x':
        if (this.obj.state.xy != null) {
          const xy = [
            Math.round(state.x / 6.5279) / 10000.0,
            this.obj.state.xy[1]
          ]
          this.checkXY(xy)
        }
      ...
      case 'y':
        if (this.obj.state.xy != null) {
          const xy = [
            this.obj.state.xy[0],
            Math.round(state.y / 6.5279) / 10000.0
          ]
          this.checkXY(xy)
        }
        break
