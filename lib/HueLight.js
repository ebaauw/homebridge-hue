...
HueLight.prototype.setConfig = function() {
  ...
  // this.subtype = this.uniqueid.split('-')[1];
...
HueLight.prototype.checkState = function(state, event) {
  for (const key in state) {
    switch (key) {
      case 'on':
        this.checkOn(state.on, state.reachable);
        break;
      // jshint -W106
      case 'all_on':
...
HueLight.prototype.checkOn = function(on, reachable) {
  ...
  let hkOn = this.obj.state.on ? 1 : 0;
  if (hkOn && !this.config.ignoreReachable && !reachable) {
...
HueLight.prototype.checkReachable = function(reachable) {
  if (!this.config.ignoreReachable) {
    return;
  }
