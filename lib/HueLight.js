...
HueLight.prototype.checkAdaptiveLighting = function () {
  if (this.al == null || !this.obj.state.on) {
    return
  }
  const state = this.al.getCt(this.hk.bri)
  if (state != null && state.ct !== this.hk.ct) {
    this.log.info(
      '%s: homekit adaptive lighting color temperature changed from %s mired to %s mired',
      this.name, this.hk.ct, state.ct
    )
    const oldCT = this.hk.ct
    this.hk.ct = state.ct
    const newCT = this.hk.ct
...
HueLight.prototype.deactivateTransitionControl = function () {
  if (this.al != null && this.al.active) {
    this.al.deactivate()
