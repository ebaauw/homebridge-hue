...
HueLight.prototype.setHK = function() {
  ...
  if (this.type === 'group') {
    ...
    this.hk.anyOn = this.state.any_on ? 1 : 0;
    // jshint +W106
  } else {
  ...
  if (this.config.ct) {
    this.hk.ct = Math.max(
      this.config.minCT, Math.min(this.config.maxCT, this.state.ct)
    );
  }
  if (this.config.xy) {
...
HueLight.prototype.heartbeat = function(obj) {
  ...
  // jshint +W106
  if (this.hk.anyOn !== old.hk.anyOn) {
    this.log.info(
      '%s: set homekit any on from %s to %s', this.name,
    	old.hk.anyOn, this.hk.anyOn
    );
    this.service.getCharacteristic(Characteristic.AnyOn)
      .updateValue(this.hk.anyOn);
  }
...
HueLight.prototype.checkState = function(state, event) {
  ...
  if (state.any_on !== undefined && state.any_on !== this.state.any_on) {
    ...
    this.state.any_on = state.any_on;
    const anyOn = this.state.any_on ? 1 : 0;
    // jshint +W106
