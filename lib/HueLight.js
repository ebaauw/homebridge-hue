...
HueLight.prototype.setCT = function(ct, callback) {
  ...
  this.log.info("%s: homekit color temperature changed from %sK to %sK", this.name,
  		this.hk.ct, ct);
  this.hk.ct = ct;
  const maxCT = this.manufacturername === "OSRAM" ? 370 : 500;
  const newCT = Math.max(153, Math.min(Math.round(1000000.0 / this.hk.ct), maxCT));
