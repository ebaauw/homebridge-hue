...
const knownLights = {
  ...
  'IKEA of Sweden': {
    ...
    models: {
      ...
      'FLOALT panel WS 30x30': { noTransition: true },
  ...
  if (this.config.windowCovering) {
    ...
    if (this.config.effects != null) {
      ...
      if (this.config.effectSpeed) {
        ...
        this.hk.effectColour1 = [255, 0, 0]
        this.hk.effectColour2 = [0, 255, 0]
        this.hk.effectColour3 = [0, 0, 255]
        this.hk.effectColour4 = [255, 255, 0]
        this.hk.effectColour5 = [255, 0, 255]
        this.hk.effectColour5 = [0, 255, 255]
      }
...
HueLight.prototype.setEffect = function (effect, on, callback) {
  ...
  const newEffect = effect === -1
    ? 'none'
    : this.config.effects[effect].toLowerCase()
  this.hk.effect = effect
  const body = { effect: newEffect }
  if (this.config.effectSpeed) {
    body.effectSpeed = this.hk.effectSpeed
    // body.effectColours = [
    //   this.hk.effectColour1,
    //   this.hk.effectColour2,
    //   this.hk.effectColour3,
    //   this.hk.effectColour4,
    //   this.hk.effectColour5,
    //   this.hk.effectColour6
    // ]
  }
  this.put(body).then(() => {
    this.obj.state.effect = newEffect
...
HueLight.prototype.setEffectSpeed = function (speed, callback) {
  ...
  this.put({ effectSpeed: speed }).then(() => {
    callback()
  }).catch((error) => {
    callback(error)
  })
