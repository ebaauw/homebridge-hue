...
function HueLight (bridge, id, obj, type) {
  ...
  if (this.config.power) {
    ...
    this.historyEntry = {time: now}
    this.serviceList.push(this.historyService)
    this.hk.totalConsumption = 0
    ...
    .on('set', (value, callback) => {
      ...
      this.service.updateCharacteristic(
        eve.Characteristic.TotalConsumption, this.hk.TotalConsumption
      )
      callback(null)
    })
...
HueLight.prototype.setConfig = function () {
  this.config = {
    ...
    power: this.obj.state.power !== undefined
  }
  if (this.config.ct) {
...
HueLight.prototype.checkPower = function (power) {
  ...
  const hkPower = this.obj.state.power
  if (this.hk.power !== hkPower) {
    if (this.hk.power !== undefined) {
      this.log.info(
        '%s: set homekit consumption from %sW to %sW', this.name,
...
HueLight.prototype.checkEntry = function (changed) {
  if (!this.config.power) {
    return
  }
  const now = moment().unix()
  ...
  if (
    changed ||
    now - this.historyEntry.time + this.bridge.state.heartrate > threshold
  ) {
    this.hk.totalConsumption += (now - this.historyEntry.time) *
                                this.historyEntry.power
    this.service.getCharacteristic(eve.Characteristic.TotalConsumption)
      .updateValue(this.hk.totalConsumption)
    this.historyEntry.time = now
    this.historyEntry.power = this.hk.power
    this.log.info(
      '%s: add history entry %j', this.name, this.historyEntry
    )
    this.historyService.addEntry(this.historyEntry)
  }
}
