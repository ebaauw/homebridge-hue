...
function HueLight (accessory, id, obj, type = 'light') {
  ...
  if (this.config.windowCovering) {
    ...
    if (this.config.valve) {
      ...
      if (this.type === 'group') {
        this.service.addOptionalCharacteristic(my.Characteristics.AnyOn)
        this.service.getCharacteristic(my.Characteristics.AnyOn)
...
HueLight.prototype.setConfig = function () {
  ...
  if (this.config.xy && !this.bridge.platform.config.forceCt) {
    this.config.ct = false
  }
  ...
  if (this.config.ct) {
    if (model.minCT != null) {
      ...
      this.obj.capabilities.control.ct !== 0
    ) {
    ...
    if (model.maxCT != null) {
      ...
      this.obj.capabilities.control.ct != null &&
      this.obj.capabilities.control.ct !== 0 &&
      this.obj.capabilities.control.ct !== 65535
    ) {
      // reported by Hue bridge
...
HueLight.prototype.identify = function (callback) {
  ...
  } else if (this.manufacturer === 'Philips') {
    alert = 'breathe'
    stop = 'stop'
  ...
  this.bridge.request('put', this.resourcePath, { alert: alert })
    .then((obj) => {
      if (stop != null) {
        setTimeout(() => {
          this.bridge.request('put', this.resourcePath, { alert: stop })
        }, 1500)
      }
      return callback()
    }).catch((err) => {
...
HueLight.prototype.setWarning = function (on, callback) {
  const alert = on ? 'select' : 'none'
  this.bridge.request('put', this.resourcePath, { alert: alert })
    .then((obj) => {
      setTimeout(() => {
        this.service.updateCharacteristic(Characteristic.On, 0)
      }, 1000)
      return callback()
    })
}
...
HueLight.prototype.setOn = function (on, callback) {
  if (!this.config.on) {
    return this.setWarning(on, callback)
  }
  on = on ? 1 : 0
...
HueLight.prototype.setStreaming = function (streaming, callback) {
  ...
  this.hk.streaming = streaming
  if (this.hk.streaming === 0) {
    this.bridge.request('put', this.resource, { stream: { active: false } })
      .then((obj) => {
        this.obj.state.streaming = false
        return callback()
      }).catch((err) => {
        return callback(err)
      })
  ...
  if (this.hk.duration > 0) {
    if (this.hk.active) {
      ...
      this.hk.autoInActiveTimeout = setTimeout(() => {
        ...
        this.request({ on: false }).then(() => {
        }).catch((err) => {
          this.log.error('%s: error %j', this.name, err)
        })
