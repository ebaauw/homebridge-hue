...
// TODO:
// - Delay calling callback from setOn(), setBri(), ... until after put() has
//   returned, so bridge/gateway error can be returned to HomeKit app.
// - Maybe setup on('get', ...) for HomeKit characteristics, to indicate non-
//   reachable accessories to HomeKit app, see:
  ...
  .then ((obj) => {
    for (const key in desiredState) {
      if (key !== 'transitiontime') {
       this.obj.state[key] = desiredState[key];
     }
    }
    if (this.type === 'group' && desiredState.on !== undefined) {
      // jshint -W106
      this.obj.state.any_on = desiredState.on;
      this.obj.state.all_on = desiredState.on;
      // jshint +W106
    }
    this.updating = false;
    return;
