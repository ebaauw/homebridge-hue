...
const knownLights = {
  '3A Smart Home DE': {
    // See: https://3asmarthome.com/products
    models: {
      'LXN-1S27LX1.0': { fix: function () { this.obj.type = 'On/Off light',  this.config.bri = false } },
      'LXN56-0S27LX1.3': { fix: function () { this.obj.type = 'On/Off light',  this.config.bri = false } }
    }
  },
  'Busch-Jaeger': {
...
HueLight.prototype.setStreaming = function (streaming, callback) {
  ...
  if (this.hk.streaming === 0) {
    this.put({ stream: { active: false } }).then((obj) => {
