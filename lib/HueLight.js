...
HueLight.prototype.getSupportedTransitionConfiguration = function (callback) {
  try {
    ...
    this.log.warn(
      '%s: cannot compute supported transition configuration: %s',
      formatError(error)
    )
    callback(error)
  }
...
HueLight.prototype.getTransitionControl = function (callback) {
  try {
    ...
    this.log.debug(
      '%s: set homekit transition control to %s', this.name, control
    )
    this.log.info(
    ...
    this.log.warn(
      '%s: cannot compute transition control: %s', formatError(error)
    )
    callback(error)
  }
}
...
HueLight.prototype.setTransitionControl = function (control, callback) {
  try {
    this.log.debug(
      '%s: homekit transition control set to %s', this.name, control
    )
    this.log.info(
    ...
    this.log.debug(
      '%s: set homekit transition control to %s', this.name, controlResponse
    )
    this.log.info(
      '%s: set homekit transition control to %j', this.name,
    ...
    this.log.warn(
      '%s: cannot handle transition control: %s', formatError(error)
    )
  }
}
...
HueLight.prototype.checkAdaptiveLighting = function () {
  ...
  const hkState = this.al.getCt(this.hk.bri)
  if (hkState == null) {
    return
  }
