...
HueLight.prototype.setCT = function (ct, callback) {
  ...
  this.deactivateTransitionControl()
...
HueLight.prototype.getSupportedTransitionConfiguration = function (callback) {
  ...
  this.log.debug(
    '%s: supported transition configuration: %s', this.name, configuration
  )
...
HueLight.prototype.getTransitionControl = function (callback) {
  ...
  this.log.debug(
    '%s: transition control: %j', this.name, this.al.parseControl(control)
  )
  this.log.debug('%s: transition control: %s', this.name, control)
  callback(null, control)
}

HueLight.prototype.setTransitionControl = function (control, callback) {
  this.log.debug('%s: set transition control to: %s', this.name, control)
  this.log.debug(
    '%s: set transition control to: %j', this.name,
    this.al.parseControlWrite(control)
  )
  const controlResponse = this.al.generateControlResponse()
  this.log.debug(
    '%s: transition control response: %j', this.name,
    this.al.parseControlResponse(controlResponse)
  )
  this.log.debug('%s: transition control response: %s', this.name, controlResponse)
  callback(null, controlResponse)
  this.service.getCharacteristic(my.Characteristics.ActiveTransitionCount)
    .updateValue(1)
}

HueLight.prototype.deactivateTransitionControl = function () {
  if (this.al != null) {
    this.al.deactivate()
    this.service.getCharacteristic(my.Characteristics.ActiveTransitionCount)
      .updateValue(0)
