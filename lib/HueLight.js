...
function HueLight(bridge, id, obj, type) {
  ...
  this.uuid_base = zigbee ? obj.uniqueid.split('-')[0] : bridge.uuid_base + this.resource;
  ...
  this.infoService
    .setCharacteristic(Characteristic.Manufacturer, zigbee ? obj.manufacturername : 'Philips')
  ...
  if (this.state.any_on !== undefined) {
    this.service.addOptionalCharacteristic(Characteristic.AnyOn);
    this.service.setCharacteristic(Characteristic.AnyOn, this.hk.any_on);
...
HueLight.prototype.refresh = function() {
  ...
  this.state.all_on = this.obj.state.all_on;
  this.state.any_on = this.obj.state.any_on;
  this.hk = {};
  if (this.state.all_on !== undefined) {
...
HueLight.prototype.heartbeat = function(obj) {
  ...
  if (this.state.all_on !== old.state.all_on) {
    this.log.debug('%s: %s all_on changed from %s to %s', this.name, this.type,
  ...
  if (this.state.any_on !== old.state.any_on) {
    this.log.debug('%s: %s any_on changed from %s to %s', this.name, this.type,
    		   old.state.any_on, this.state.any_on);
  }
  if (this.hk.any_on !== old.hk.any_on) {
    this.log.info('%s: set homekit any on from %s to %s', this.name,
...
HueLight.prototype.setOn = function(on, callback) {
  ...
  this.bridge.request('put', this.resourcePath, {on: newOn})
  .then(function(obj) {
    this.state.on = newOn;
    if (this.state.any_on !== undefined) {
      this.state.any_on = newOn;
      this.state.all_on = newOn;
    }
    return callback();
  }.bind(this))
  .catch(function(err) {
...
HueLight.prototype.setAnyOn = function(on, callback) {
  ...
  this.log.info('%s: homekit any on changed from %s to %s', this.name, this.hk.any_on, on);
  this.hk.on = on;
  const newOn = this.hk.on ? true : false;
  this.bridge.request('put', this.resourcePath, {on: newOn})
  .then(function(obj) {
...
HueLight.prototype.setBri = function(bri, callback) {
  ...
  this.bridge.request('put', this.resourcePath, {bri: newBri})
  .then(function(obj) {
...
HueLight.prototype.setColorTemperature = function(ct, callback) {
  ...
  this.bridge.request('put', this.resourcePath, {ct: newCT})
  .then(function(obj) {
    this.state.ct = newCT;
    if (this.bridge.platform.config.ct) {
      this.service.setCharacteristic(Characteristic.CT, this.state.ct);
...
HueLight.prototype.setCT = function(ct, callback) {
  ...
  this.log.info('%s: homekit color temperature changed from %s to %s', this.name,
  		this.state.ct, ct);
  this.bridge.request('put', this.resourcePath, {ct: ct})
  .then(function(obj) {
    this.state.ct = ct;
    this.hk.ct = this.colorTemperature(this.state.ct);
    this.service.setCharacteristic(Characteristic.ColorTemperature, this.hk.ct);
...
HueLight.prototype.setHue = function(hue, callback) {
  ...
  this.bridge.request('put', this.resourcePath, {xy: [newXY[0], newXY[1]]})
  .then(function(obj) {
    this.state.xy = newXY;
    return callback();
  }.bind(this))
...
HueLight.prototype.setSat = function(sat, callback) {
  ...
  this.hk.sat = sat;
  const newXY = invHueSat(this.hk.hue, this.hk.sat, this.obj.modelid);
  this.bridge.request('put', this.resource, {xy: [newXY[0], newXY[1]]})
  .then(function(obj) {
    this.state.xy = newXY;
    return callback();
  }.bind(this))
