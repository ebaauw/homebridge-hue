      ...
      'GLEDOPTO': {                               // Issue #244
        ...
        fix: function () {
          if (
            ...
            this.model = 'RGB+CCT'
            const other = this.serialNumber + '-0a'
            for (const id in this.bridge.fullState.lights) {
              const light = this.bridge.fullState.lights[id]
              if (light.uniqueid === other) {
                this.model = 'RGBW'
...
function HueLight (bridge, id, obj, type) {
  ...
  if (this.config.consumption) {
    this.service.addCharacteristic(eve.Characteristic.TotalConsumption)
    this.historyService = new HistoryService('energy', {
      displayName: this.serialNumber, log: this.log
    }, {
      storage: 'fs', disableTimer: true
    })
    this.serviceList.push(this.historyService)
    this.history = {}
    this.checkConsumption(this.obj.state.consumption)
    // this.addEntry()
...
HueLight.prototype.setConfig = function () {
  this.config = {
    ...
    consumption: this.obj.state.consumption !== undefined
  }
  if (this.config.ct) {
...
HueLight.prototype.addEntry = function () {
  if (!this.config.consumption) {
    return
  }
  if (this.history.consumption != null) {
    const delta = Math.round(
      1000.0 * (this.obj.state.consumption - this.history.consumption)
    )
  ...
  for (const key in state) {
    switch (key) {
      ...
      case 'colormode':
        ...
        break
      case 'consumption':
        this.checkConsumption(state.consumption)
        break
      case 'ct':
...
HueLight.prototype.checkConsumption = function (consumption) {
  if (!this.config.consumption) {
    return
  }
  if (this.obj.state.consumption !== consumption) {
    this.log.debug(
      '%s: %s consumption changed from %s to %s', this.name, this.type,
      this.obj.state.consumption, consumption
    )
    this.obj.state.consumption = consumption
  }
  const hkConsumption = this.obj.state.consumption / 1000.0
  if (this.hk.consumption !== hkConsumption) {
    if (this.hk.consumption !== undefined) {
      this.log.info(
        '%s: set homekit total consumption from %s kWh to %s kWh', this.name,
        this.hk.consumption, hkConsumption
      )
    }
    this.hk.consumption = hkConsumption
    this.service.getCharacteristic(eve.Characteristic.TotalConsumption)
      .updateValue(this.hk.consumption)
  }
}
...
HueLight.prototype.checkCurrent = function (current) {
  if (!this.config.consumption) {
    return
  }
  if (this.obj.state.current !== current) {
    this.log.debug(
      '%s: %s current changed from %s to %s', this.name, this.type,
...
HueLight.prototype.checkPower = function (power) {
  if (!this.config.consumption) {
    return
  }
  if (this.obj.state.power !== power) {
    this.log.debug(
      '%s: %s power changed from %s to %s', this.name, this.type,
      this.obj.state.power, power
    )
    this.obj.state.power = power
  }
  const hkPower = this.obj.state.power / 10.0
  if (this.hk.power !== hkPower) {
    if (this.hk.power !== undefined) {
      this.log.info(
        '%s: set homekit current consumption from %s W to %s W', this.name,
        this.hk.power, hkPower
      )
    }
    this.hk.power = hkPower
    this.service.getCharacteristic(eve.Characteristic.CurrentConsumption)
      .updateValue(this.hk.power)
  }
}
...
HueLight.prototype.checkVoltage = function (voltage) {
  if (!this.config.consumption) {
    return
  }
  if (this.obj.state.voltage !== voltage) {
    this.log.debug(
      '%s: %s voltage changed from %s to %s', this.name, this.type,
      this.obj.state.voltage, voltage
    )
    this.obj.state.voltage = voltage
  }
  const hkVoltage = this.obj.state.voltage / 100.0
