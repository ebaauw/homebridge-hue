...
HueLight.prototype.setConfig = function() {
  ...
  switch (this.config.manufacturer) {
    ...
    case 'dresden elektronik':
      switch (this.config.model) {
        case 'FLS-PP3', 'FLS-PP3 White':  // FLS-PP lp
          // Include extension for Dresden Elektronik lights, as the Hue bridge
          // sees these as multiple lights with the same Zigbee ID.
          this.serialNumber = this.obj.uniqueid;
        break;
        default:
          this.log.error(
            '%s: warning: %s: unknown %s light model', this.name,
