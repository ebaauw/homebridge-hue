  ...
  if (this.config.ct) {
    if (this.bridge.platform.config.ct) {
      ...
      this.service.getCharacteristic(Characteristic.CT)
        ...
        .setProps({minValue: this.config.minCT, maxValue: this.config.maxCT});
    } else {
      ...
      this.service.getCharacteristic(Characteristic.ColorTemperature)
        ...
        .setProps({
          minValue: this.colorTemperature(this.config.maxCT),
          maxValue: this.colorTemperature(this.config.minCT)
        });
    }
  }
...
HueLight.prototype.invColorTemperature = function(kelvin) {
  return Math.max(
    this.config.minCT,
    Math.min(Math.round(1000000.0 / kelvin), this.config.maxCT)
  );
};
