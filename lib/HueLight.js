...
HueLight.prototype.checkReachable = function (reachable) {
  ...
  if (this.config.wallSwitch && !this.obj.state.reachable) {
    if (this.service.testCharacteristic(Characteristic.On)) {
      if (this.hk.on !== noResponse) {
        this.log.info(
          '%s: set homekit on from %s to %s', this.name,
          this.hk.on, noResponse
        )
        this.hk.on = noResponse
        this.service.getCharacteristic(Characteristic.On)
          .updateValue(this.hk.on)
      }
    } else if (this.service.testCharacteristic(Characteristic.Active)) {
      if (this.hk.active !== noResponse) {
        this.log.info(
          '%s: set homekit active from %s to %s', this.name,
          this.hk.active, noResponse
        )
        this.hk.active = noResponse
        this.service.getCharacteristic(Characteristic.Active)
          .updateValue(this.hk.active)
