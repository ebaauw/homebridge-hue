...
function HueLight(bridge, id, obj, type) {
  ...
  this.uuid_base = zigbee ?
    obj.uniqueid.split('-')[0] : bridge.uuid_base + this.resource;
  if (obj.manufacturername === 'dresden elektronik') {
    // Include extension for Dresden Elektronik lights, as the Hue bridge treats
    // these as multiple lights with the same Zigbee ID.
  ...
  this.infoService
    .updateCharacteristic(
      Characteristic.Manufacturer, zigbee ? obj.manufacturername : 'Philips'
    )
    .updateCharacteristic(Characteristic.Model, zigbee ? obj.modelid : obj.type)
    .updateCharacteristic(Characteristic.SerialNumber, this.uuid_base);
   if (
     this.state.bri === undefined && this.state.ct === undefined &&
     this.state.xy === undefined
   ) {
  ...
  if (this.state.ct !== undefined) {
    if (obj.manufacturername === 'OSRAM') {
      ...
      obj.type === 'Color temperature light'
    ) {
      this.maxCT =  454;
    } else {
      this.maxCT =  500;
...
HueLight.prototype.heartbeat = function(obj) {
  ...
  if (
    this.state.reachable !== undefined &&
    this.state.reachable !== old.state.reachable
  ) {
    this.log.debug(
...
HueLight.prototype.put = function(key, value) {
  // jshint -W106
  if (this.desiredState) {
  ...
  setTimeout(function() {
    ...
    .then (function(obj) {
      ...
      if (this.state.any_on !== undefined && desiredState.on !== undefined) {
