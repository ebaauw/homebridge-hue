...
function HueSchedule (bridge, id, obj, type) {
  ...
  this.service = new my.Service.Resource(this.name, this.resource)
  this.service.getCharacteristic(my.Characteristic.Enabled)
    .updateValue(this.hk.enabled)
    .on('set', this.setEnabled.bind(this))
  if (this.type === 'rule') {
    this.service
      .updateCharacteristic(my.Characteristic.LastTriggered, this.hk.lasttriggered)
      .updateCharacteristic(
        my.Characteristic.TimesTriggered, this.hk.timestriggered
      )
  }
  this.service
  ...
  if (this.bridge.platform.config.resource) {
    this.service
      .updateCharacteristic(my.Characteristic.Resource, this.resource)
...
HueSchedule.prototype.heartbeat = function (beat, obj) {
  ...
  if (this.obj.lasttriggered !== old.obj.lasttriggered) {
    ...
    this.service
      .updateCharacteristic(
        my.Characteristic.LastTriggered, this.hk.lasttriggered
      )
      .updateCharacteristic(
        my.Characteristic.TimesTriggered, this.hk.timestriggered
      )
