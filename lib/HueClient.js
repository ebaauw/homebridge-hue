...
class HueClient extends homebridgeLib.HttpClient {
  ...
  async get (resource) {
    ...
    if (response.errors.length > 0) {
      const error = new Error(`cannot GET ${response.request.resource}`)
      throw error
    }
  ...
  /** Issue a PUT request to `/api/`_username_`/`_resource_.
    *
    * HueClient throttles the number of PUT requests to 20 per seconds (or 1
    * per seconds for `/groups` resources), delaying the request when needed.
  ...
  async put (resource, body) {
    ...
    const timeout = resource.startsWith('/groups')
    ...
    const response = await this.request('PUT', resource, body)
    if (Object.keys(response.success).length === 0) {
      const error = new Error(`cannot PUT ${response.request.resource}`)
      throw error
    }
    return response
  }
  ...
  async post (resource, body) {
    const response = await this.request('POST', resource, body)
    if (response.errors.length > 0) {
      const error = new Error(`cannot POST ${response.request.resource}`)
      throw error
    }
    return response
  }
  ...
  async delete (resource, body) {
    const response = await this.request('DELETE', resource, body)
    if (response.errors != null && response.errors.length > 0) {
      const error = new Error(`cannot DELETE ${response.request.resource}`)
      throw error
    }
    return response
  }
  ...
  async createuser (application) {
    ...
    try {
      ...
      this._options.username = response.success.username
    ...
    try {
      ...
      if (error.code === 'ECONNRESET' || error.statusCode === 503) {
        if (error.request != null) {
          error.message += ' - retry in ' +
            this.platform.config.waitTimeResend + 'ms'
          this.emit('error', error)
