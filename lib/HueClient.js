...
const { HttpClient, OptionParser, semver, timeout } = require('homebridge-lib')
const os = require('os')

const deconzMacPrefixes = ['00212E']
const hueMacPrefixes = ['001788', 'ECB5FA']
...
class HueClient extends HttpClient {
  ...
  /** Check if bridgeid is for a Hue bridge.
    * @param {string} bridgeid - The bridge ID.
    * @returns {boolean}
    */
  static isHueBridgeId (bridgeid) {
    return hueMacPrefixes.includes(bridgeid.slice(0, 6))
  }
  ...
  constructor (params = {}) {
    ...
    if (HueClient.isHueBridgeId(_options.config.bridgeid)) {
      if (semver.gte(_options.config.apiversion, '1.24.0')) {
        _options.https = true
      }
      _options.isHue = true
    } else if (HueClient.isDeconzBridgeId(_options.config.bridgeid)) {
    ...
    if (_options.https && !_options.forceHttp) {
      ...
      if (semver.gte(_options.config.apiversion, '1.48.0')) {
