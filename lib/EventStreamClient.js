        ...
        for (const key of Object.keys(data)) {
          ...
          switch (key) {
            ...
            case 'relative_rotary':
              state.rotaryevent = value.last_event.action === 'start' ? 1 : 2
              state.expectedrotation = value.last_event.rotation.steps *
                (value.last_event.rotation.direction === 'clock_wise' ? 1 : -1)
              state.expectedeventduration = value.last_event.rotation.duration
              state.lastupdated = obj.creationtime.slice(0, -1)
              break
            case 'motion':
