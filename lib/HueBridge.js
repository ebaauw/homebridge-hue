...
class HueBridge {
  ...
  constructor (platform, host, bridge) {
    ...
    this.schedules = {}
    this.rules = {}
    this.areaMotionUuidToId = {}
    this.areaMotionIndex = 900000
    this.v2OwnerNameCache = {}
    this.areaMotionUnavailable = {}
  ...
  async getFullState () {
    ...
    await this.addAreaMotionSensors(state)
    this.fullState = state
    return state
  }

  async getAreaMotionResources () {
    const resources = []
    for (const type of ['convenience_area_motion', 'security_area_motion']) {
      if (this.areaMotionUnavailable[type]) {
        continue
      }
      try {
        const response = await this.get(`/resource/${type}`)
        ...
        if (error.request != null) {
          if ([403, 404, 405].includes(error.statusCode)) {
            this.areaMotionUnavailable[type] = true
            this.log.debug(
              '%s: %s resources not available: %s', this.name, type,
              formatError(error)
            )
          } else {
            this.log.warn(
              '%s: failed to retrieve %s resources: %s', this.name, type,
              formatError(error)
            )
          }
        } else {
  ...
  getAreaMotionSensorId (resource) {
    if (resource.id_v1 != null) {
      const match = resource.id_v1.match(/\/sensors\/(\d+)/)
      if (match != null) {
        return match[1]
      }
    }
    if (this.areaMotionUuidToId[resource.id] != null) {
      return this.areaMotionUuidToId[resource.id]
    }
    const id = String(this.areaMotionIndex++)
    this.areaMotionUuidToId[resource.id] = id
    return id
  }

  async resolveOwnerName (owner) {
    if (owner == null || owner.rid == null || owner.rtype == null) {
      return null
    }
    const key = owner.rtype + ':' + owner.rid
    if (Object.prototype.hasOwnProperty.call(this.v2OwnerNameCache, key)) {
      return this.v2OwnerNameCache[key]
    }
    let name = null
    let cache = true
    try {
      const response = await this.get(`/resource/${owner.rtype}/${owner.rid}`)
      ...
      if (error.request != null) {
        if ([403, 404, 405].includes(error.statusCode)) {
          this.log.debug(
            '%s: owner %s not resolved: %s', this.name, key, formatError(error)
          )
        } else {
          cache = false
          this.log.warn(
            '%s: failed to resolve owner %s: %s', this.name, key,
            formatError(error)
          )
        }
      } else {
        cache = false
        this.log.warn(
          '%s: failed to resolve owner %s: %s', this.name, key, formatError(error)
        )
      }
    }
    if (cache) {
      this.v2OwnerNameCache[key] = name
    }
    return name
  }

  formatHueTimestamp (isoString) {
  ...
  async resolveAreaMotionName (resource, state) {
    const defaultSuffix = resource.type === 'security_area_motion'
      ? 'Security Motion'
      : 'Motion'
    if (resource.id_v1 != null) {
      const match = resource.id_v1.match(/\/(groups|zones)\/(\d+)/)
      if (match != null) {
        const group = state.groups?.[match[2]]
        if (group?.name != null) {
          return group.name + ' ' + defaultSuffix
        }
      }
    }
    const ownerName = await this.resolveOwnerName(resource.owner)
    if (ownerName != null) {
      return ownerName + ' ' + defaultSuffix
    }
    return (resource.type === 'security_area_motion'
      ? 'Security Area Motion '
      : 'Convenience Area Motion ') + resource.id.slice(0, 4)
  }

  async areaMotionResourceToSensor (resource, state) {
    const id = this.getAreaMotionSensorId(resource)
    const name = await this.resolveAreaMotionName(resource, state)
    const sensitivity = resource.sensitivity?.sensitivity
    const sensitivityMax = resource.sensitivity?.sensitivity_max
    const sensor = {
      name,
      type: 'CLIPPresence',
      modelid: resource.type,
      manufacturername: 'Signify Netherlands B.V.',
      uniqueid: resource.id,
      swversion: this.version,
      config: {
        on: resource.enabled ?? false,
        reachable: true
      },
      state: {
        presence: resource.motion?.motion_report?.motion ?? false,
        lastupdated: this.formatHueTimestamp(
          resource.motion?.motion_report?.changed
        )
      },
    ...
    if (sensitivityMax != null) {
      sensor.config.sensitivitymax = sensitivityMax
      sensor.config.sensitivity = sensitivity
    }
    return { id, sensor }
  }

  async addAreaMotionSensors (state) {
    const sensors = state.sensors ?? {}
    const resources = await this.getAreaMotionResources()
    for (const resource of resources) {
      const { id, sensor } = await this.areaMotionResourceToSensor(resource, state)
      sensors[id] = sensor
      if (this.fullState?.sensors != null) {
        this.fullState.sensors[id] = sensor
      }
    }
    state.sensors = sensors
  }

  async fetchAreaMotionSensorState () {
    if (this.fullState == null) {
      return {}
    }
    if (this.fullState.sensors == null) {
      this.fullState.sensors = {}
    }
    const state = {}
    const resources = await this.getAreaMotionResources()
    for (const resource of resources) {
      const { id, sensor } = await this.areaMotionResourceToSensor(
        resource, this.fullState
      )
      state[id] = sensor
      this.fullState.sensors[id] = sensor
    }
  ...
  async heartbeatSensors (beat) {
    ...
    const areaSensors = await this.fetchAreaMotionSensorState()
    for (const id in areaSensors) {
      sensors[id] = areaSensors[id]
    }
    for (const id in sensors) {
