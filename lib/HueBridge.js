...
const { formatError, semver, timeout } = require('homebridge-lib')
...
const { HueAccessory } = HueAccessoryModule
const HueClient = require('./HueClient')
const { HueSchedule } = HueScheduleModule
const WsMonitor = require('./WsMonitor')

module.exports = { setHomebridge, HueBridge }
  ...
  switch (this.model) {
    ...
    case 'deCONZ': // deCONZ rest api
      ...
      if (this.platform.config.homebridgeDeconz === '') {
        this.log.warn(
          '%s: warning: support for deCONZ will be deprecated in favour of Homebridge deCONZ',
          this.name
        )
      }
      if (!semver.satisfies(this.version, recommendedVersion)) {
...
HueBridge.prototype.exposeResources = async function (obj) {
  ...
  for (const key in obj.resourcelinks) {
    ...
    if (
      ...
      (
        key === this.platform.config.homebridgeDeconz &&
        link.name === 'homebridge-deconz' &&
      ...
      this.log.debug(
        '%s: /resourcelinks/%d: %d entries exposed by Homebridge %s',
        this.name, key, link.links.length,
        link.name === 'homebridge-deconz' ? 'deCONZ' : 'Hue2'
      )
      for (const resource of link.links) {
        const type = resource.split('/')[1]
        const id = resource.split('/')[2]
...
HueBridge.prototype.listen = function () {
  ...
  const ws = new WsMonitor({ host, retryTime: 15 })
  ws
    .on('error', (error) => {
      this.log.warn(
