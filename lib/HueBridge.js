...
HueBridge.prototype.getConfig = async function () {
  ...
  return new Promise((resolve, reject) => {
    this.hueClient.config().then((obj) => {
...
HueBridge.prototype.exposeBridge = function (obj) {
  ...
  switch (this.model) {
    ...
    case 'BSB002': // Philips Hue v2 (square) bridge;
      ...
      if (semver.satisfies(this.version, '>=1.36.0')) {
        this.philips = 'Signify Netherlands B.V.'
      }
      this.config.link = semver.satisfies(this.version, '<1.31.0')
    ...
    case 'deCONZ': // deCONZ rest api
      if (obj.bridgeid === '0000000000000000') {
        return new Promise((resolve, reject) => {
          ...
          setTimeout(() => {
            resolve(this.getConfig().then((obj) => {
              return this.exposeBridge(obj)
            }))
          }, 60000)
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    this.request('POST', '/', { devicetype: devicetype })
      .then((obj) => {
        ...
        if (this.isDeconz) {
          this.log.info(
            '%s: unlock gateway to create a user', this.name
          )
        } else {
          this.log.info(
...
HueBridge.prototype.retryExposeResources = async function () {
  this.log.info(
  ...
  try {
    ...
    if (error.code === 'ECONNRESET') {
      ...
      this.log.debug(
        ...
        this.name, this.type, error.code
      )
      await delay(this.platform.config.waitTimeResend)
    ...
    if (error.code != null) {
      this.log.error(
        ...
        error.code, this.platform.maskHost(this.host)
      )
    } else {
      this.log.error('%s: error: %s', this.name, error.message)
    }
    throw new Error()
  }
}
