...
HueBridge.prototype.getSerialNumber = function (id, obj) {
  if (obj.uniqueid) {
    const a = obj.uniqueid.match(/(..:..:..:..:..:..:..:..)-..(-....)?/)
    if (a != null) {
      // ZigBee device
      return a[1].replace(/:/g, '').toUpperCase()
    }
  }
  if (obj.action != null) {
    return this.serialNumber + '-G' + id
  }
  if (
    ...
    return this.serialNumber + '-' +
           obj.uniqueid.split('-')[0].split('/')[2]
  }
  return this.serialNumber + '-' + id
}
...
HueBridge.prototype.exposeSensor = function (id, obj) {
  const serialNumber = this.getSerialNumber(id, obj)
