...
const repeaterTypes = [
  'Unknown', // Issue #2490 in deCONZ v2.05.74.
...
HueBridge.prototype.accessories = async function () {
  ...
  try {
    ...
    if (error.message !== 'unknown bridge') {
      this.log.error('%s: %s', this.name, error.message)
    }
  }
...
HueBridge.prototype.getConfig = async function () {
  ...
  try {
    ...
    if (error.message) {
      this.log.error('%s: %s - retrying in 15s', this.name, error.message)
    }
  ...
  if (obj.type[0] === 'Z') {
    ...
    if (a != null) {
      ...
      if (
        ...
        obj.manufacturername === this.philips && obj.modelid === 'SML001' &&
...
HueBridge.prototype.exposeLights = function (lights) {
  if (this.platform.config.lights) {
    for (const id in lights) {
      ...
      } else if (repeaterTypes.includes(light.type)) {
...
HueBridge.prototype.heartbeat = async function (beat) {
  if (beat % this.state.heartrate === 0 && this.request) {
    ...
    try {
      ...
      if (error.message) {
        this.log.debug('%s: heartbeat error: %s', this.name, error)
      }
    }
  }
  if (beat % 600 === 0 && this.request) {
    try {
      ...
      this.log.error('%s: heartbeat error:', this.name, error)
    }
  }
}
...
HueBridge.prototype.request = async function (method, resource, body) {
  ...
  try {
    ...
    if (
      ...
      (error.message != null && error.message.includes(': http status 503'))
    ) {
      this.log.debug(requestMsg)
      this.log.debug(
        '%s: %s communication error: %s - retrying in %dms',
        this.name, this.type, error.message, this.platform.config.waitTimeResend
      )
    ...
    if (error.code != null && !error.code.startsWith('ERR_')) {
      this.log.error(
        '%s: %s communication error: %s', this.name, this.type, error.message
      )
    } else {
      this.log.error('%s: error: %s', this.name, error)
    }
    throw new Error()
  }
}
