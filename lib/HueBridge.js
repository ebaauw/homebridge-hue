...
HueBridge.prototype.exposeResources = function (obj) {
  ...
  if (this.isHue && this.platform.config.scenes) {
    ...
    for (const key in obj.scenes) {
      ...
      if (scene.appdata.version === 1 && scene.appdata.data != null) {
        // Scene created by Hue app for a room.
        const a = /^.{5}_r([0-9][0-9])_d[0-9][0-9]$/.exec(scene.appdata.data)
        if (a === null || a[1] === null) {
          continue
        }
        const id = parseInt(a[1])
        if (obj.groups[id] == null) {
          continue
        }
        this.log.debug('%s: /scenes/%s: room: %d', this.name, key, id)
