...
HueBridge.prototype.getConfig = async function () {
  if (this.hueClient == null) {
    ...
    this.hueClient.on('response', (id, code, message, body) => {
      this.log.debug(
        '%s: %s request %d: %d %s', this.name, this.type, id, code, message
      )
    })
...
HueBridge.prototype.listen = function () {
  ...
  this.ws.on('message', (data, flags) => {
    try {
      ...
      this.log.warn('%s: websocket error %s', this.name, formatError(e))
    }
  })
...
HueBridge.prototype.request = async function (method, resource, body) {
  if (this.hueClient == null) {
    ...
    this.hueClient.on('request', (id, method, resource, body, url) => {
      if (body == null) {
        ...
        this.log.debug(
          '%s: %s request %d: %s %s %s', this.name, this.type,
          id, method, resource, body
        )
      }
    })
    this.hueClient.on('response', (id, code, message, body) => {
    ...
    this.hueClient.on('error', (/* error, id, method, resource, body, url */) => {
      ...
      // }
      // this.log.warn(
      //   '%s: %s request %d: communication error: %s', this.name, this.type,
  ...
  requestMsg = util.format(
    '%s: %s request %d: %s %s', this.name, this.type,
    requestNumber, method.toUpperCase(), resource
  )
  if (body != null) {
  ...
  try {
    ...
    if (error.code === 'ECONNRESET' || error.statusCode === 503) {
      this.log.debug(
        '%s: %s request %d: communication error: %s - retrying in %dms',
        this.name, this.type, requestNumber, formatError(error),
    ...
    this.log.warn(
      '%s: %s request %d: communication error: %s', this.name, this.type,
      requestNumber, formatError(error)
    )
    throw error
  }
}
