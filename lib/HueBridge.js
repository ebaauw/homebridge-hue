...
function setHomebridge (homebridge, _my, _eve) {
  HueLightModule.setHomebridge(homebridge, _my)
...
function HueBridge (platform, host) {
  ...
  this.name = this.platform.maskHost(this.host)
...
HueBridge.prototype.exposeResources = function (obj) {
  ...
  for (const key in obj.resourcelinks) {
    ...
    if (link.name === 'homebridge-hue' && link.links && link.description) {
      let list = link.description.toLowerCase()
      switch (list) {
        case 'blacklist':
        case 'lightlist':
        case 'whitelist':
        ...
        default:
          this.log.warn(
            '%s: /resourcelinks/%d: please change description "%s" to "blacklist"',
  ...
  try {
    ...
    if (sensor.service) {
      ...
      if (accessory) {
        ...
        if (
          accessory.historyService && accessory.type.history === 'weather' &&
          (sensor.type.key === 'temperature' || sensor.type.key === 'pressure')
        ) {
          sensor.type.history = 'weather'
          sensor.historyService = accessory.historyService
          sensor.historyEntry = accessory.historyEntry
        } else if (
          sensor.historyService && sensor.type.history === 'weather' &&
          (accessory.type.key === 'temperature' || accessory.type.key === 'pressure')
        ) {
          accessory.type.history = 'weather'
          accessory.historyService = sensor.historyService
          accessory.historyEntry = sensor.historyEntry
        }
      } else {
