...
HueBridge.prototype.accessories = function() {
  ...
  return this._request('get', '/config', null).then(function(obj) {
    ...
    // this.service.getCharacteristic(Characteristic.Search)
    //   .updateValue(this.hk.search)
    //   .on('get', this.getSearch.bind(this))
    ...
    if (this.platform.config.linkbutton) {
      ...
      this.switchService.getCharacteristic(Characteristic.LabelIndex)
        .setValue(1);
    }
...
HueBridge.prototype.refresh = function() {
  ...
  this.hk.search = this.state.search ? 1 : 0;
...
HueBridge.prototype.heartbeatConfig = function() {
  return this.request('get', '/config').then(function(obj) {
    ...
    // if (this.hk.search !== old.hk.search) {
    //   this.log(
    //     '%s: set homekit search from %s to %s', this.name,
    //     old.hk.search, this.hk.search
    //   );
    //   this.service.updateCharacteristic(Characteristic.Search, this.hk.search);
    // }
...
// HueBridge.prototype.getSearch = function(callback) {
//   this.request('get', '/lights/new').then(function(obj) {
//     this.state.search = obj.lastscan === 'active';
//     return callback(this.state.search);
//   }.bind(this))
//   .catch(function (err) {
//     return callback(new Error(err));
//   }.bind(this));
...
//   search = search ? 1 : 0;
//   if (search === this.hk.search) {
//     return callback();
//   }
//   this.log.info(
//     '%s: homekit search changed from %s to %s', this.name,
//     this.hk.search, search
//   );
//   this.hk.search = search;
//   if (!this.hk.search) {
//     return callback();
//   }
//   this.request('post', '/lights')
//   .then(function() {
