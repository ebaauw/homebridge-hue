...
HueBridge.prototype.createResources = function() {
  ...
  return this.request('get', '/').then(function(obj) {
    this.obj = obj.config;
    ...
    if (this.platform.config.sensors) {
      for (const id in obj.sensors) {
        ...
        if (this.blacklist.sensors[id]) {
          ...
          this.config.nativeHomeKit &&
          sensorObj.type[0] === 'Z' && sensorObj.manufacturername === 'Philips'
        ) {
          this.log.debug('%s: /sensors/%d: exposed by bridge', this.name, id);
        } else if (
          this.platform.config.excludeSensorTypes[sensorObj.type] || (
            sensorObj.type.substring(0, 4) === 'CLIP'  &&
            this.platform.config.excludeSensorTypes.CLIP
          )
        ) {
          this.log.debug(
            '%s: /sensors/%d: %s excluded', this.name, id, sensorObj.type
          );
        } else if (
