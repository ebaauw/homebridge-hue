    ...
    if (link.name === 'homebridge-hue' && link.links && link.description) {
      ...
      for (const resource of link.links) {
        ...
        if (list === 'multiclip') {
          ...
          // TODO: save first resource (or Room group?)
  ...
  if (a != null) {
    ...
    if (this.multiclip[id] == null) {
      if (
        ...
        this.log.warn(
          '%s: /sensors/%d: warning: old multiCLIP setup will be deprecated',
          this.name, id
        )
        serialNumber += '-' + obj.uniqueid.split('-')[0].split('/')[2]
      } else {
        serialNumber += '-' + id
      }
    } else {
      serialNumber += '-M' + this.multiclip[id]
    }
  }
...
HueBridge.prototype.exposeLight = function (id, obj) {
  ...
  if (this.multilight[id] == null) {
    ...
    serialNumber = this.serialNumber + '-M' + this.multilight[id]
  }
  let accessory = this.accessoryMap[serialNumber]
  if (accessory == null) {
    accessory = new HueAccessory(this, serialNumber)
