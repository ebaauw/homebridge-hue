...
HueBridge.prototype.accessories = function() {
  ...
  .then(function(obj) {
    ...
    switch (obj.modelid) {
      ...
      default:
        this.log.warn(
          '%s: warning: ignoring unknown bridge type %j',
          this.name, obj
        );
        break;
    }
  ...
  this.ws.on('message', (data, flags) => {
    try {
      ...
      if (obj.e === 'changed' && obj.t === 'event') {
        ...
        // this.log.debug('%s: %s: %j', this.name, resource, state);
        let a;
        switch (obj.r) {
          case 'lights':
      ...
      this.log.error('%s: %s', this.name, e);
    }
  });

  this.ws.on('error', (error) => {
    this.log.error('%s: %s', this.name, error);
  });

  this.ws.on('close', () => {
    this.log.debug(
      '%s: connection closed - retry in %d seconds', this.name,
      this.platform.config.timeout
    );
    setTimeout(this.listen, this.platform.config.timeout * 1000);
  });
};
