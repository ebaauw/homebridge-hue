  ...
  .then(function(obj) {
    ...
    switch (obj.modelid) {
      ...
      case 'BSB002':                // Philips Hue v2 (square) bridge;
        ...
        this.log.info(
          '%s: %s bridge v%s, api v%s', this.name,
          obj.modelid, obj.swversion, obj.apiversion
        );
        versions = obj.apiversion.split('.');
        major = Number(versions[0]);
        minor = Number(versions[1]);
        if (major !== 1 || minor < 15 || minor > 20) {
          this.log.warn(
            '%s: warning: unsupported api v%s', this.name, obj.apiversion
          );
        }
        break;
      case 'deCONZ':                // deCONZ rest api
        ...
        this.log.info(
          '%s: %s gateway v%s, api v%s', this.name,
          obj.modelid, obj.swversion, obj.apiversion
        );

        if (obj.apiversion !== '1.0.2') {
          this.log.warn(
            '%s: warning: unsupported api v%s', this.name, obj.apiversion
          );
        }
        if (obj.swversion !== this.platform.packageConfig.engines.deconz) {
          this.log.warn(
            '%s: warning: not using recommended deCONZ v%s', this.name,
            this.platform.packageConfig.engines.deconz
          );
        }
        break;
      default:
...
HueBridge.prototype.refresh = function() {
  ...
  this.hk.lastupdated = this.obj.UTC ?
    String(new Date(this.obj.UTC)).substring(0, 24) : 'n/a';
      ...
      if (obj.e === 'changed' && obj.t === 'event') {
        ...
        if (a) {
          if (obj.state !== undefined) {
            a.stateChangedEvent(obj.state);
          }
          if (obj.config !== undefined) {
            a.configChangedEvent(obj.config);
          }
        }
      }
    } catch(e) {
