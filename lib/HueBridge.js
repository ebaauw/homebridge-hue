  ...
  .then(function(obj) {
    ...
    switch (obj.modelid) {
      ...
      case 'BSB002':                // Philips Hue v2 (square) bridge;
        ...
        if (!semver.satisfies(obj.apiversion, recommendedVersion)) {
          this.log.warn(
      ...
      case 'deCONZ':                // deCONZ rest api
        ...
        if (!semver.valid(obj.swversion)) {
          // Fix use of leading 0 in deCONZ minor version.
          const versions = obj.swversion.split('.');
          const major = Number(versions[0]);
          const minor = Number(versions[1]);
          const patch = Number(versions[2]);
          obj.swversion = major + '.' + minor + '.' + patch;
        }
        this.log.info(
          '%s: %s %s v%s, api v%s', this.name, obj.modelid, this.type,
          obj.swversion, obj.apiversion
        );
        if (!semver.satisfies(obj.swversion, recommendedVersion)) {
          this.log.warn(
