...
HueAccessory.prototype.exposeSensorResources = function () {
  ...
  if (this.resources.sensors.other.length > 0) {
    ...
    if (
      ...
      const service = new eve.Service.AirPressureSensor('Dummy Pressure')
      const perms = [Characteristic.Perms.READ, Characteristic.Perms.HIDDEN]
      service.getCharacteristic(eve.Characteristic.AirPressure)
        .setProps({ perms: perms })
        .updateValue(1000)
      service.getCharacteristic(eve.Characteristic.Elevation)
        .setProps({ perms: perms })
        .updateValue(0)
