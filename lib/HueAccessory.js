    ...
    if (this.bridge.platform.config.scenes) {
      if (obj.scenes != null) {
        ...
        for (const scene of obj.scenes) {
          const resource = '/groups/' + id + '/scenes/' + scene.id
          const sceneName = obj.name + ' ' + scene.name
          this.log.debug('%s: %s: "%s"', this.bridge.name, resource, sceneName)
          const service = new my.Services.Resource(sceneName, 'scene' + scene.id)
          service.getCharacteristic(my.Characteristics.Enabled)
            .setValue(0)
            .on('set', (value, callback) => {
              this.log('%s: recall scene %j', this.bridge.name, sceneName)
              this.bridge.request('put', resource + '/recall').then((obj) => {
                setTimeout(() => {
                  service.updateCharacteristic(my.Characteristics.Enabled, 0)
                }, 1000)
                callback()
              })
            })
...
HueAccessory.prototype.exposeSensorResources = function () {
  ...
  if (this.resources.sensors.other.length > 0) {
    ...
    if (
      ...
      this.resources.sensors.other.length === 1 &&
      this.resources.sensors.other[0].obj.type.substr(-11) === 'Temperature'
    ) {
      this.log.debug(
        '%s: /sensors/0: CLIPHumidity "Dummy Humidity"', this.bridge.name
      )
      const service = new Service.HumiditySensor('Dummy Humidity')
    ...
    if (
      ...
      this.resources.sensors.other[0].obj.type.substr(-11) === 'Temperature' && (
        this.resources.sensors.other.length === 1 ||
        this.resources.sensors.other[1].obj.type.substr(-8) === 'Humidity'
      )
    ) {
      this.log.debug(
