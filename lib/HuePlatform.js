...
function setHomebridge (homebridge) {
  ...
  // Source: as exposed by Eve Weather and Eve Degree.
  Characteristic.AirPressure = function () {
    Characteristic.call(
      this, 'Air Pressure', Characteristic.AirPressure.UUID
    )
    this.setProps({
      format: Characteristic.Formats.UINT16,
      unit: 'hPa',
  ...
  // Source: as exposed by Eve Degree.
  ...
  Service.AirPressureSensor.UUID = 'E863F00A-079E-48FF-8F27-9C2605A29F52'

  // Custom HomeKit Weather service.
  // Source: as exposed by Eve Weather.
  Service.Weather = function (displayName, subtype) {
    Service.call(this, displayName, Service.Weather.UUID, subtype)
    this.addCharacteristic(Characteristic.CurrentTemperature)
    this.addCharacteristic(Characteristic.Humidity)
    this.addCharacteristic(Characteristic.AirPressure)
  }
  util.inherits(Service.Weather, Service)
  Service.Weather.UUID = 'E863F001-079E-48FF-8F27-9C2605A29F52'

  // Custom HomeKit characteristic for LastActivation.
  // Source: as exposed by Eve Door.
  Characteristic.LastActivation = function () {
    Characteristic.call(
      this, 'Last Activation', Characteristic.LastActivation.UUID
    )
    this.setProps({
  ...
  // Custom HomeKit characteristic for TimesOpened.
  // Source: as exposed by Eve Door.
  Characteristic.TimesOpened = function () {
    Characteristic.call(
      this, 'Times Opened', Characteristic.TimesOpened.UUID
    )
    this.setProps({
      format: Characteristic.Formats.UINT32,
      perms: [Characteristic.Perms.READ, Characteristic.Perms.NOTIFY]
  ...
  // Custom HomeKit characteristic for ResetTotal.
  // Source: as exposed by Eve Door.
  Characteristic.ResetTotal = function () {
    Characteristic.call(
      this, 'Reset Total', Characteristic.ResetTotal.UUID
    )
    this.setProps({
      format: Characteristic.Formats.UINT32,
      perms: [
  ...
  // Custom HomeKit characteristic for ???.
  // Source: as exposed by Eve Door.
  Characteristic.Char118 = function () {
    Characteristic.call(
      this, 'Char 118', Characteristic.Char118.UUID
    )
    this.setProps({
      format: Characteristic.Formats.UINT32,
      perms: [
  ...
  util.inherits(Characteristic.Char118, Characteristic)
  ...
  Characteristic.Char119 = function () {
    Characteristic.call(
      this, 'Char 119', Characteristic.Char119.UUID
    )
    this.setProps({
      format: Characteristic.Formats.UINT32,
      perms: [
  ...
  util.inherits(Characteristic.Char119, Characteristic)
...
function HuePlatform (log, configJson, api) {
  ...
  for (const key in configJson) {
    ...
    switch (key.toLowerCase()) {
      ...
      case 'nativehomekit':
        this.log.warn(
          'config.json: warning: %s: deprecated - use %s', key,
          'nativeHomeKitlights and/or nativeHomeKitSensors'
        )
      ...
      case 'philipslights':
        this.log.warn(
        'config.json: warning: %s: deprecated - use NativeHomeKitLights', key
      )
        this.config.nativeHomeKitLights = !value
        break
      case 'platform':
