...
//
// Config parameters:
//   host:		Hue bridge hostname (or IP address) [default: discover bridge].
//   user:		Hue bridge whitelisted user.
//   heartrate:		Heartbeat interval in seconds [default: 5].
//   timeout:		Timeout interval for bridge discovery in seconds [default: 5].
//   lights:		Create accessories for Philips lights [default: false].
//			Accessories for non-Philips lights are always created.
//   groups:		Create accessories for groups [default: false].
//   sensors:		Create accessories for sensors [default: false].
//   schedules:		Create accessories for schedules [default: false]. 
//
// Todo:
// - Bridge discovery using local UPnP.
// - Dynamic homebridge accessories.
// - Support creating user (bridge password).
...
HuePlatform.prototype.accessories = function(callback) {
  ...
  this.findBridges(function(json) {
    if (json) {
      for (let obj of json) {
        this.log.debug("probing bridge at ", obj.internalipaddress);
