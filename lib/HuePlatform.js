...
HuePlatform.prototype.meethueSearch = function () {
  ...
  const d = deferred()
  const list = []
  this.request(name, 'https://discovery.meethue.com/', '')
    .then((responseBody) => {
  ...
  this.request(
    ...
    if (Array.isArray(responseBody)) {
      for (const bridge of responseBody) {
        ...
        if (bridge.internalport !== 80) {
          host = host + ':' + bridge.internalport
        }
        this.log.debug(
          '%s: found gateway %s at %s', name,
          bridge.id.toUpperCase(), this.maskHost(host)
        )
        list.push(host)
      }
    }
...
HuePlatform.prototype.upnpSearch = function () {
  ...
  socket.on('message', (message, rinfo) => {
    ...
    if (
      ...
      let host = response.location.split('/')[2]
      if (host.split(':')[1] === '80') {
        host = host.split(':')[0]
      }
