...
function setHomebridge(homebridge) {
  ...
  Characteristic.LastTriggered = function() {
    Characteristic.call(this, 'Last Triggered', Characteristic.LastTriggered.UUID);
    this.setProps({
      format: Characteristic.Formats.INT,
      perms: [Characteristic.Perms.READ, Characteristic.Perms.NOTIFY]
    });
  ...
  Characteristic.TimesTriggered = function() {
    Characteristic.call(this, 'Times Triggered', Characteristic.TimesTriggered.UUID);
    this.setProps({
      format: Characteristic.Formats.INT,
      perms: [Characteristic.Perms.READ, Characteristic.Perms.NOTIFY]
    });
  ...
  Characteristic.ColorTemperature = function() {
    Characteristic.call(this, 'Color Temperature', 'A18E5901-CFA1-4D37-A10F-0071CEEEEEBD');
    this.setProps({
      format: Characteristic.Formats.INT,
      // unit: 'kelvin',
      minValue: 2000,
  ...
  for (const key in config) {
    ...
    switch (key.toLowerCase()) {
      ...
      case 'sensors':
        ...
        break;
      case 'clipsensors':
        this.log.error('config.json: %s: warning: key has been deprecated', key);
      ...
      case 'excludesensortypes':
        if (Array.isArray(value)) {
          ...
          this.log.error('config.json: %s: warning: ignoring non-array value', key);
        }
        break;
      case 'lowbattery':
        this.config.lowBattery = this.toIntBetween(value, 0, 100, this.config.waittime);
        break;
      case 'lights':
      ...
      case 'rules':
        ...
        break;
      case 'heartrate':
        this.config.heartrate = this.toIntBetween(value, 1, 30, this.config.heartrate);
        break;
      case 'waittimeupdate':
        this.config.waitTimeUpdate = this.toIntBetween(value, 20, 500, this.config.waittime);
        break;
      case 'timeout':
        this.config.timeout = 1000 * this.toIntBetween(value, 5, 30, this.config.timeout);
        break;
      case 'parallelrequests':
        this.config.parallelRequests = this.toIntBetween(value, 1, 30, this.config.timeout);
        break;
      case 'waittimeresend':
        this.config.waitTimeResend = this.toIntBetween(value, 100, 1000, this.config.waittime);
        break;
      default:
...
HuePlatform.prototype.toIntBetween = function(value, minValue, maxValue, defaultValue) {
  const n = Number(value);
  if (isNaN(n) || n !== Math.floor(n) || n < minValue || n > maxValue) {
    return defaultValue;
  }
