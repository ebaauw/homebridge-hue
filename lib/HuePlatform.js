...
function maskHost(host) {
  ...
  return elt.length === 4 ? [elt[0], '***', '***', elt[3]].join('.') : host;
}

function maskConfigJson(configJson) {
  let json = {};
  Object.assign(json, configJson);
  if (configJson.host) {
    json.host = maskHost(configJson.host);
  }
  for (const id in configJson.hosts) {
    json.hosts[id] = maskHost(configJson.hosts[id]);
  }
  return json;
}

function maskUsernames(usernames, json) {
...
function HuePlatform(log, configJson, api) {
  ...
  this.log.debug(
    'config.json: %s', maskUsernames(usernames, maskConfigJson(this.configJson))
  );
}
...
HuePlatform.prototype.dump = function() {
  ...
  let obj = {
    ...
    config: maskConfigJson(this.configJson),
    bridges: []
  };
  for (const bridgeId in this.configJson.users) {
    usernames.push(this.configJson.users[bridgeId]);
  ...
  for (const bridge of this.bridges) {
    ...
    state.config.ipaddress = maskHost(state.config.ipaddress);
    state.config.gateway = maskHost(state.config.gateway);
    if (state.config.proxyaddress !== 'none') {
      state.config.proxyaddress = maskHost(state.config.proxyaddress);
    }
  ...
  try {
    zlib.gzip(maskUsernames(usernames, obj), (err, data) => {
      if (err) {
    ...
    if (responseBody.length === 0) {
      ...
      for (const bridge of responseBody) {
        this.log.debug(
          ...
          bridge.id.toUpperCase(), maskHost(bridge.internalipaddress)
        );
...
HuePlatform.prototype.upnpSearch = function() {
  ...
  socket.on('message', (message, rinfo) => {
    ...
    if (
      ...
      if (!map[ipaddress]) {
        this.log.debug(
          'upnp search: found bridge %s at %s', bridgeid, maskHost(ipaddress)
        );
