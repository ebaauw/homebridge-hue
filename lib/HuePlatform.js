...
function setHomebridge(homebridge) {
  ...
  // Standard homekit characteristic for Color Temperature in Mired.
...
function HuePlatform(log, config, api) {
  ...
  const major = Number(process.version.substr(1, 1));
  if (major !== 6 && major !== 7) {
    this.log.warn('warning: %s: unsupported node version', process.version);
  }
