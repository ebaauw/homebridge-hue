...
HueSensor.prototype.checkValue = function (value) {
  ...
  if (this.hk[this.type.key] !== hkValue) {
    ...
    this.service
      .updateCharacteristic(this.type.Characteristic, this.hk[this.type.key])
    if (
      this.type.key === 'temperature' &&
      this.accessory.temperatureCharacteristic != null
    ) {
      this.accessory.temperatureCharacteristic
        .updateValue(this.hk[this.type.key])
    }
    this.addEntry(true)
    if (this.type.key === 'power' && this.accessory.resource.config.outlet) {
...
HueSensor.prototype.checkSunrise = function (sunrise) {
  ...
  const hkSunrise =
    (this.obj.state.sunrise && this.obj.state.sunrise !== 'none')
      // ? String(new Date(this.obj.state.sunrise + 'Z')).substring(0, 24) : 'n/a'
      ? String(new Date(this.obj.state.sunrise)).substring(0, 24) : 'n/a'
...
HueSensor.prototype.checkSunset = function (sunset) {
  ...
  const hkSunset =
    (this.obj.state.sunset && this.obj.state.sunset !== 'none')
      // ? String(new Date(this.obj.state.sunset + 'Z')).substring(0, 24) : 'n/a'
      ? String(new Date(this.obj.state.sunset)).substring(0, 24) : 'n/a'
...
HueSensor.prototype.setLocked = function (locked, callback) {
  ...
  this.bridge.request('put', this.resource + '/config', { locked: hueLocked })
    .then((obj) => {
...
HueSensor.prototype.setMirroring = function (mirroring, callback) {
  ...
  this.bridge.request('put', this.resource + '/config', { displayflipped: displayflipped })
    .then((obj) => {
      this.obj.config.displayflipped = displayflipped
      return callback()
    }).catch((err) => {
...
HueSensor.prototype.setTargetHeatingCoolingState = function (
  ...
  if (this.model === 'SPZB0001') {
    ...
    this.bridge.request('put', this.resource + '/config', { mode: mode })
      .then((obj) => {
        this.obj.config.mode = mode
        return callback()
      }).catch((err) => {
