...
//
// HueSensor provides support for sensors in the Philips Hue bridge.
...
function setHomebridge(homebridge) {
  ...
  hueSensorTypes = {
    ...
    ZLLTemperature: {	// 1.4
      ...
      name:		"temperature",
      unit:		"˚C",
      homekitValue:	function(v) {return v ? v / 100 : 0;}
    },
    ZLLLightLevel: {	// 2.7
...
function HueSensor(bridge, id, obj) {
  ...
  this.uuid_base = bridge.uuid_base + "/sensors/" + id;
  this.url = "/sensors/" + id;
  this.obj = obj;
  this.refresh();
...
HueSensor.prototype.heartbeat = function(obj) {
  ...
  if (this.type.key === "buttonevent") {
    if (this.obj.state.lastupdated !== old.obj.state.lastupdated && this.hk.value !== 0) {
      this.log.debug("%s: sensor buttonevent %d on %s",
      		     this.name, this.value, this.obj.state.lastupdated);
      this.log.info("%s: set homekit state from %s to %s on",
      		    this.name, old.hk.value, this.hk.value, this.hk.lastupdated);
    ...
    if (this.value !== old.value) {
      ...
      this.log.info("%s: set homekit %s from %s%s to %s%s on %s",
      		    this.name, this.type.name, old.hk.value, this.type.unit,
  ...
  if (this.obj.config.on !== old.obj.config.on) {
    this.log.debug("%s: sensor on changed from %s to %s",
    		   this.name, old.config.on, this.obj.config.on);
...
HueSensor.prototype.identify = function(callback) {
  ...
  this.bridge.request("put", this.url + "/config", "{\"alert\":\"select\"}", function(obj) {
    return callback();
  }.bind(this));
}
...
HueSensor.prototype.setValue = function(value, callback) {
  ...
  this.log.info("%s: homekit %s changed from %s%s to %s%s",
  		this.name, this.type.name, this.hk.value, this.type.unit,
  		value, this.type.unit);
  let newValue = this.type.bridgeValue(value);
  this.bridge.request("put", this.url + "/state",
  		      "{\"" + this.type.key + "\":" + newValue + "}", function(obj) {
    if (!obj) {
