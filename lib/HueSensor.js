    ...
    case 'Daylight':
      if (
        ...
        if (this.bridge.model !== 'deCONZ') {
          const period = daylightEvents[obj.state.status].period
          obj.state.lightlevel = daylightPeriods[period].lightlevel
          obj.state.dark = daylightPeriods[period].dark
        } else {
          obj.state.lightlevel = obj.state.daylight ? 65535 : 0
          obj.state.dark = !obj.state.day
        }
