...
function HueSensor(bridge, id, obj) {
  ...
  const zigbee = this.obj.type[0] === 'Z';
  ...
  switch(this.obj.type) {
    case 'ZGPSwitch':   // 1.1 - Hue Tap
      this.uuid_base = obj.uniqueid.split('-')[0];
      ['1', '2', '3', '4'].map(function(button) {
        ...
        service.getCharacteristic(Characteristic.ProgrammableSwitchEvent)
          .setProps({minValue: Characteristic.ProgrammableSwitchEvent.SINGLE_PRESS,
                     maxValue: Characteristic.ProgrammableSwitchEvent.SINGLE_PRESS})
          .setValue(-1);
      }.bind(this));
      this.service = new Service.StatefulProgrammableSwitch(this.name);
      this.type = {
        Characteristic: Characteristic.ProgrammableSwitchOutputState,
        props:          {minValue: 0, maxValue: 4, stepValue: 1,
    ...
    case 'CLIPSwitch': // 2.1
      /* falls through */
    default:
      this.log.error('%s: unsupported sensor type %s', this.name, this.obj.type);
      return;
  }
  this.serviceList.push(this.service);
  this.infoService
    .setCharacteristic(Characteristic.Manufacturer, zigbee ?
                                                    obj.manufacturername : 'Service')
  ...
  if (this.hk.dark !== undefined) {
    ...
    this.service.setCharacteristic(Characteristic.Dark, this.hk.dark);
  }
  if (this.hk.daylight !== undefined) {
    this.service.addOptionalCharacteristic(Characteristic.Daylight);
    this.service.setCharacteristic(Characteristic.Daylight, this.hk.daylight);
  }
  this.service.addOptionalCharacteristic(Characteristic.Enabled);
  this.service.setCharacteristic(Characteristic.Enabled, this.hk.enabled);
  ...
  this.service.setCharacteristic(Characteristic.LastUpdated, this.hk.lastupdated);
  this.service.addOptionalCharacteristic(Characteristic.StatusActive);
  this.service.setCharacteristic(Characteristic.StatusActive, this.hk.enabled);
  ...
  this.service.setCharacteristic(Characteristic.Resource, this.resource);
  if (this.obj.config.battery) {
    this.batteryService = new Service.BatteryService(this.name);
    this.serviceList.push(this.batteryService);
    this.batteryService.setCharacteristic(Characteristic.BatteryLevel, this.hk.battery);
    this.batteryService.setCharacteristic(Characteristic.StatusLowBattery, this.hk.lowBattery);
    this.batteryService.setCharacteristic(Characteristic.ChargingState, this.hk.charging);
  }
}
...
HueSensor.prototype.heartbeat = function(obj) {
  ...
  if (this.type.key === 'buttonevent') {
    if (this.obj.state.lastupdated !== old.obj.state.lastupdated) {
      ...
      this.log.debug('%s: set homekit %s from %s to %s on %s', this.name,
      ...
      if (this.hk.value !== 0 && action !== undefined) {
        this.log.info('%s: homekit button %d %s', this.name, this.hk.value,
                      {0: 'single press', 1: 'double press', 2: 'long press'}[action]);
        this.serviceList[this.hk.value]
          .setCharacteristic(Characteristic.ProgrammableSwitchEvent, action);
        setTimeout(function () {
          this.serviceList[this.hk.value]
  ...
  if (this.obj.state.dark !== old.obj.state.dark) {
    this.log.debug('%s: sensor dark changed from %s to %s',
    		   this.name, old.obj.state.dark, this.obj.state.dark);
  }
  if (this.hk.dark !== old.hk.dark) {
    this.log.info('%s: set homekit dark from %s to %s',
                  this.name, old.hk.dark, this.hk.dark);
    this.service.setCharacteristic(Characteristic.Dark, this.hk.dark);
  }
  if (this.obj.state.daylight !== old.obj.state.daylight) {
    this.log.debug('%s: sensor daylight changed from %s to %s',
    		   this.name, old.obj.state.daylight, this.obj.state.daylight);
  }
  if (this.hk.daylight !== old.hk.daylight) {
    this.log.info('%s: set homekit daylight from %s to %s',
                  this.name, old.hk.daylight, this.hk.daylight);
    this.service.setCharacteristic(Characteristic.Daylight, this.hk.daylight);
  }
  if (this.obj.state.lastupdated !== old.obj.state.lastupdated) {
    this.service.setCharacteristic(Characteristic.LastUpdated,
    				   this.hk.lastupdated);
  }
  ...
  if (this.hk.enabled !== old.hk.enabled) {
    this.log.info('%s: set homekit enabled from %s to %s',
      		    this.name, old.hk.enabled, this.hk.enabled);
    this.service.setCharacteristic(Characteristic.Enabled, this.hk.enabled);
    this.service.setCharacteristic(Characteristic.StatusActive, this.hk.enabled);
  }
  ...
  if (this.hk.battery !== old.hk.battery) {
    ...
    this.batteryService
      .setCharacteristic(Characteristic.BatteryLevel, this.hk.battery)
      .setCharacteristic(Characteristic.StatusLowBattery, this.hk.lowBattery);
  }
};
...
HueSensor.prototype.setEnabled = function(enabled, callback) {
  ...
  this.log.info('%s: homekit enabled changed from %s to %s', this.name,
  	            this.hk.enabled, enabled);
  this.hk.enabled = enabled;
  const on = this.hk.enabled === 1;
