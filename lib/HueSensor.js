...
function HueSensor (bridge, id, obj) {
  ...
  switch (this.obj.type) {
    ...
    case 'CLIPWater':
      ...
      break
    case 'ZHAConsumption':
      this.subtype = this.obj.uniqueid.split('-').slice(1).join('-')
      // falls through
  ...
  if (this.service) {
    if (this.type.Characteristic) {
      ...
      if (this.type.history != null) {
        ...
        const filename = this.obj.type[0] === 'Z'
        ...
        this.historyService = new HistoryService(this.type.history, this, {
          ...
          filename: 'history_' + filename + '.json'
        })
  ...
  switch (this.type.history) {
    ...
    case 'energy':
      ...
      if (this.history.consumption != null) {
        ...
        this.history.entry.power = delta * 6.0
      }
    ...
    case 'weather':
      ...
      if (changed || this.type.key !== 'humidity') {
        return
      }
      break
    default:
