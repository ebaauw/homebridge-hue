...
function HueSensor (accessory, id, obj) {
  ...
  this.infoService = this.accessory.getInfoService(this.obj)
  ...
  if (this.service) {
    if (this.type.Characteristic) {
      ...
      if (this.type.history != null) {
        this.historyService = this.accessory.getHistoryService(
          this.type.history, this.type.key
        )
        this.history = this.accessory.history
        if (this.type.history !== this.accessory.history.type) {
          // Some-one beat us.
...
HueSensor.prototype.addEntry = function (changed) {
  ...
  switch (this.history.type) {
    ...
    case 'energy':
      if (this.history.key === 'power') {
        this.history.consumption += this.history.power *
                                    (now - this.history.time)
        this.history.power = this.hk.power
        this.history.time = now
      }
      if (changed) {
      ...
      if (this.history.key === 'consumption') {
    ...
    case 'weather':
      ...
      if (changed || this.type.key !== this.history.key) {
        return
      }
      break
    default:
