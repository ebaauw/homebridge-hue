  ...
  if (this.service) {
    if (this.type.Characteristic) {
      ...
      if (this.type.history != null) {
        this.historyService = new HistoryService(this.type.history, this)
        this.historyService.log = {debug: () => {}}
        const now = moment().unix()
        this.historyState = {
          offset: now,
          entry: {time: now - 600 + 2 * this.bridge.state.heartrate}
        }
        this.serviceList.push(this.historyService)
        switch (this.type.history) {
          case 'door':
            this.hk.timesOpened = 0
            this.service.addCharacteristic(Characteristic.LastActivation)
            this.service.addCharacteristic(Characteristic.TimesOpened)
...
HueSensor.prototype.checkEntry = function (changed) {
  if (this.type.history == null) {
    return
  }
  const now = moment().unix()
  switch (this.type.history) {
    ...
    case 'motion':
      if (changed) {
        this.hk.lastActivation = now - this.historyState.offset
        this.service.updateCharacteristic(
          Characteristic.LastActivation, this.hk.lastActivation
        )
        this.historyState.entry.time = now
        this.historyState.entry.status = this.hk[this.type.key]
        this.log.info(
          '%s: add history entry %j', this.name, this.historyState.entry
        )
        this.historyService._addEntry(this.historyState.entry)
      }
      break
    case 'weather':
      const key = this.type.key === 'temperature' ? 'temp' : this.type.key
      this.historyState.entry[key] = this.hk[this.type.key]
      break
    default:
      return
  }
  if (now - this.historyState.entry.time + this.bridge.state.heartrate > 600) {
    this.historyState.entry.time = now
    this.log.info(
      '%s: add history entry %j', this.name, this.historyState.entry
    )
