  ...
  switch(this.obj.type) {
    ...
    case 'ZHASwitch':
      if (
        ...
        this.createButton(2, 'Dim Up', true);
        this.createButton(3, 'Dim Down', true);
        this.createButton(4, 'Previous', true);
        this.createButton(5, 'Next', true);
        this.service = new Service.Resource(this.name);
        this.type = {
          key:            'buttonevent',
          homekitValue:   function(v) {return Math.floor(v / 1000);},
        ...
        // ubisys D1 dimmer
        const endpoint = this.obj.uniqueid.split('-')[1];
        const n = endpoint - 1;
        const i = (endpoint - 2) * 3;
        this.createButton(i + 1, 'On/Off ' + n);
        this.createButton(i + 2, 'Dim Up ' + n, true, true);
        this.createButton(i + 3, 'Dim Down ' + n, true, true);
        this.service = new Service.Resource(this.name, endpoint);
        this.type = {
          key:            'buttonevent',
          homekitValue:   function(v) {return Math.floor(v / 1000);},
          homekitAction:  hkZLLSwitchAction
        ...
        this.log.warn(
          '%s: %s: warning: Xiaomi Mi smart cube not yet supported',
          this.bridge.name, this.resource
        );
        const endpoint = this.obj.uniqueid.split('-')[1];
        if (endpoint === '02') {
          this.createButton(1, 'Gesture');
          this.service = new Service.Resource(this.name, endpoint);
          this.type = {
            key:            'buttonevent',
            homekitValue:   function() {return undefined;},
            homekitAction:  function() {return undefined;}
          };
        } else {
          this.createButton(2, 'Rotate');
          this.service = new Service.Resource(this.name, endpoint);
          this.type = {
            key:            'buttonevent',
            homekitValue:   function() {return undefined;},
    ...
    case 'ZHAOpenClose':
      if (
        this.obj.manufacturername === 'LUMI' && (
          ...
          this.obh.modelid === 'lumi.sensor_magnet'
        )
      ) {
    ...
    case 'ZHAPressure':
      ...
      /* falls through */
    case 'CLIPPressure':
      this.service = new Service.AirPressureSensor(this.name);
      this.type = {
...
HueSensor.prototype.createButton = function(
  buttonIndex, buttonName, longPress, noSinglePress
) {
...
HueSensor.prototype.stateChangedEvent = function(state) {
  for (const key in state) {
    if (key === this.type.key) {
      ...
      const hklastupdated = state.lastupdated === 'none' ?
        'n/a' : String(new Date(state.lastupdated)).substring(0, 24);
      if (key === 'buttonevent') {
        this.log.debug(
          '%s: event: sensor buttonevent %d on %s', this.name,
          state.buttonevent, state.lastupdated
        );
        ...
        if (value !== undefined && this.value !== value) {
          this.log.debug(
            '%s: event: sensor %s changed from %s to %s on %s', this.name,
            this.type.key, this.value, value, state.lastupdated
          );
          this.log.info(
            '%s: set homekit %s from %s%s to %s%s on %s', this.name,
            this.type.name, this.hk.value, this.type.unit,
            hkvalue, this.type.unit, hklastupdated
          );
          this.service
      ...
      switch (key) {
        ...
        case 'daylight':
          ...
          if (hkdaylight !== this.hk.daylight) {
            ...
            this.hk.daylight = hkdaylight;
            this.service
              .updateCharacteristic(Characteristic.Dark, this.hk.daylight);
          }
          break;
        default:
          this.log.warn(
            '%s: ignore unsupported state.%s event', this.name, key
          );
          break;
      }
    }
  ...
  for (const key in config) {
    switch (key) {
      ...
      case 'reachable':
        if (
          !this.type.ignoreReachable &&
          config.reachable !== this.obj.config.reachable
        ) {
          this.log.debug(
      ...
      default:
        this.log.warn(
          '%s: ignore unsupported config.%s event', this.name, key
        );
        break;
    }
  }
