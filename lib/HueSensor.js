...
function HueSensor (bridge, id, obj) {
  ...
  if (this.service) {
    ...
    if (this.obj.config.offset !== undefined) {
      ...
      this.checkOffset(this.obj.state.offset)
...
HueSensor.prototype.checkOffset = function (offset) {
  if (this.obj.config.offset !== offset) {
    this.log.debug(
      '%s: sensor offset changed from %s to %s', this.name,
      this.obj.config.offset, offset
    )
    this.obj.config.offset = offset
  }
  let hkOffset = Math.round(this.obj.config.offset / 10) / 10
  if (hkOffset < -5.0) {
    hkOffset = -5.0
  } else if (hkOffset > 5.0) {
    hkOffset = 5.0
  }
  if (this.hk.offset !== hkOffset) {
    if (this.hk.offset !== undefined) {
      this.log.info(
        '%s: set homekit offset from %s to %s', this.name,
