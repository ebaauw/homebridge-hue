  ...
  switch (this.obj.type) {
    ...
    case 'ZHATemperature':
      if (
        ...
        this.obj.manufacturername === 'Develco Products A/S' && (
...
HueSensor.prototype.checkButtonevent = function (
  ...
  if (event || this.obj.state.lastupdated !== lastupdated) {
    ...
    const action = this.type.homekitAction(
      buttonevent, this.obj.state.buttonevent, this.repeat.includes(buttonIndex)
    )
...
HueSensor.prototype.setTargetHeatingCoolingState = function (
  ...
  this.put('/config', { scheduleron: hueScheduleron }).then((obj) => {
