  ...
  switch(this.obj.type) {
    ...
    case 'ZHASwitch':
      if (
        ...
        // ubisys D1 dimmer
        let ep = this.obj.uniqueid.split('-')[1];
        const n = ep - 1;
        const i = (ep - 2) * 3;
        this.createButton(i + 1, 'Button ' + n);
        this.createButton(i + 2, 'Dim Up ' + n, true, true);
        this.createButton(i + 3, 'Dim Down ' + n, true, true);
        break;  // Don't create Resource Service.
      } else if (
        ...
        let ep = this.obj.uniqueid.split('-')[1];
        if (ep !== 2) {
          break; // Only create HueResource Service for endpoint 2.
        }
      } else {
        this.log.warn(
          '%s: %s: warning: unknown %s sensor %j',
...
HueSensor.prototype.getServices = function() {
  let serviceList = [this.infoService];
  for (let service of this.serviceList) {
    serviceList.push(service);
  }
  if (this.batteryService) {
    serviceList.push(this.batteryService);
  }
  return serviceList;
};
