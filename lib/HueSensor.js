...
function HueSensor (accessory, id, obj) {
  ...
  switch (this.obj.type) {
    ...
    case 'CLIPConsumption':
      this.service = this.accessory.lightService == null
        ? new my.Service.Resource(this.name, this.subtype)
  ...
  if (this.service) {
    ...
    if (this.obj.config.locked !== undefined) {
      this.service.addOptionalCharacteristic(my.Characteristic.Locked)
      this.checkLocked(this.obj.config.locked)
      this.service.getCharacteristic(my.Characteristic.Locked)
...
HueSensor.prototype.checkValve = function (valve) {
  ...
  const hkValve = Math.round(this.obj.state.valve / 2.54)
  if (this.hk.valvePosition !== hkValve) {
    ...
    this.hk.valvePosition = hkValve
    this.service.getCharacteristic(eve.Characteristic.ValvePosition)
...
HueSensor.prototype.checkHeatSetPoint = function (heatsetpoint) {
  ...
  let hkTargetTemperature = Math.round(this.obj.config.heatsetpoint / 50) / 2
...
HueSensor.prototype.checkLocked = function (locked) {
  ...
  if (this.hk.locked !== hkLocked) {
    ...
    this.hk.locked = hkLocked
    this.service
      .updateCharacteristic(my.Characteristic.Locked, this.hk.locked)
  }
}
