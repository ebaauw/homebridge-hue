...
//
// TODO
// - Put the three services for a Hue Motion Sensor, ZLLPresence, ZLLLightLevel,
//   and ZLLTemperature, in a single accessory (using the Zigbee ID without the
//   extension).
  ...
  if (this.type.key === 'buttonevent') {
    if (this.obj.state.lastupdated !== old.obj.state.lastupdated) {
      ...
      if (this.hk.value !== 0 && action !== undefined) {
        ...
        setTimeout(function () {
          this.serviceList[this.hk.value]
            .updateCharacteristic(Characteristic.ProgrammableSwitchEvent, -1);
        }.bind(this), 20);
      }
    }
  } else {
