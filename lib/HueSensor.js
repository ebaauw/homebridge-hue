...
function HueSensor (bridge, id, obj) {
    ...
    case 'ZHASwitch':
      if (
        ...
        if (button >= 1 && button <= 4) {
          this.createButton(button, this.subtype, SINGLE_LONG)
        }
        this.type = {
          key: 'buttonevent',
          homekitValue: function (v) { return Math.floor(v / 1000) },
          ...
          this.type = {
            ...
            homekitAction: function (v) {
              return Math.abs(v) < 4500
                ? Characteristic.ProgrammableSwitchEvent.SINGLE_PRESSLONG
                : Math.abs(v) < 9000
  ...
  if (this.service) {
      ...
      if (this.type.history != null) {
        ...
        switch (this.type.history) {
          case 'door':
            ...
            this.historyService.addCharacteristic(eve.Characteristic.ResetTotal)
            ...
            break
          case 'motion':
            // this.service.addCharacteristic(eve.Characteristic.LastActivation)
            this.historyState.entry.status = 0
            break
          case 'weather':
            this.historyState.entry.temp = 0
            this.historyState.entry.humidity = 0
            this.historyState.entry.pressure = 0
            break
          default:
            break
        }
        ...
        this.historyService.addEntry(this.historyState.entry)
      }
    ...
    if (this.obj.config.duration !== undefined) {
      this.service.addOptionalCharacteristic(eve.Characteristic.Duration)
      this.checkDuration(this.obj.config.duration)
      if (this.type.readonlyDuration) {
        this.service.getCharacteristic(eve.Characteristic.Duration).setProps(
          {perms: [Characteristic.Perms.READ, Characteristic.Perms.NOTIFY]}
        )
      } else {
    ...
    if (this.obj.config.sensitivity !== undefined) {
      ...
      this.service.addOptionalCharacteristic(eve.Characteristic.Sensitivity)
...
HueSensor.prototype.checkEntry = function (changed) {
  ...
  if (
    changed ||
    now - this.historyState.entry.time + this.bridge.state.heartrate > threshold
  ) {
...
HueSensor.prototype.setSensitivity = function (sensitivity, callback) {
  ...
  this.hk.sensitivity = {0: 7, 1: 4, 2: 0}[sensitivity]
  this.bridge.request(
