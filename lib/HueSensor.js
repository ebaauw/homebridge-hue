...
function HueSensor (bridge, id, obj) {
  ...
  if (this.obj.type[0] === 'Z') {
    ...
    if (
      ...
      this.serialNumber = this.bridge.serialNumber + '-' +
                          this.obj.uniqueid.split('-')[0].split('/')[2]
      this.subtype = this.obj.uniqueid.split('-')[1]
    } else {
      this.model = this.obj.type
      // this.serialNumber = this.bridge.serialNumber + '-' + id
  ...
  if (this.service) {
    if (this.type.Characteristic) {
      ...
      if (this.type.history != null) {
        ...
        // Force history entry for weather on first heartbeat, after all
        // resources have been initialised.
        this.historyEntry = {time: now - 600 + this.bridge.state.heartrate}
        ...
        switch (this.type.history) {
          case 'door':
            ...
            // falls through
          case 'motion':
            this.historyEntry.status = 0
            break
          case 'weather':
            this.historyEntry.temp = 0
            this.historyEntry.humidity = 0
            this.historyEntry.pressure = 0
            break
          default:
            break
        }
...
HueSensor.prototype.checkValue = function (value) {
  ...
  if (this.hk[this.type.key] !== hkValue) {
    ...
    if (this.duration > 0 && hkValue === 0) {
      this.log.debug(
        '%s: keep homekit %s on %s%s for %ss on %s', this.name,
      ...
      this.durationTimer = setTimeout(() => {
        this.log.info(
          '%s: set homekit %s from %s%s to %s%s, %ss after %s',
...
HueSensor.prototype.checkEntry = function (changed) {
  ...
  switch (this.type.history) {
    ...
    case 'motion':
      ...
      this.historyEntry.status = this.hk[this.type.key]
      break
    case 'weather':
      changed = false
      const key = this.type.key === 'temperature' ? 'temp' : this.type.key
      this.historyEntry[key] = this.hk[this.type.key]
      break
    default:
      return
  }
  if (
    changed ||
    now - this.historyEntry.time + this.bridge.state.heartrate > 600
  ) {
