...
class HueSensor {
  ...
  constructor (accessory, id, obj) {
    ...
    if (this.obj.resource != null) {
      this.resource = this.obj.resource
    }
    if (this.obj.v2Resource != null) {
      this.v2Resource = this.obj.v2Resource
    }
    this.serviceList = []
  ...
  setEnabled (enabled, callback) {
    ...
    if (this.v2Resource != null) {
      const body = { type: this.v2Resource.type, enabled: on }
      this.bridge.put(this.resource, body).then(() => {
        this.obj.config.on = on
        this.service
          .updateCharacteristic(Characteristic.StatusActive, this.hk.enabled)
        return callback()
  ...
  setSensitivity (sensitivity, callback) {
    ...
    if (this.v2Resource != null) {
      const body = {
        type: this.v2Resource.type,
        sensitivity: { sensitivity: hueSensitivity }
      }
      this.bridge.put(this.resource, body).then(() => {
  ...
  put (resource, body) {
    if (this.v2Resource != null && resource === '/config') {
      const payload = { type: this.v2Resource.type }
      if (body.on != null) {
        payload.enabled = body.on
      }
      if (body.sensitivity != null) {
        payload.sensitivity = { sensitivity: body.sensitivity }
      }
      return this.bridge.put(this.resource, payload)
    }
