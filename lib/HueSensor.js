...
HueSensor.prototype.checkBoost = function (boost) {
  if (this.obj.config.boost !== boost) {
    this.log.debug(
      '%s: sensor boost changed from %j to %j', this.name,
      this.obj.config.boost, boost
    )
    this.obj.config.boost = boost
  }
  const hkTargetHeatingCoolingState = this.obj.config.boost
    ? Characteristic.TargetHeatingCoolingState.HEAT
    : this.obj.config.off
      ? Characteristic.TargetHeatingCoolingState.OFF
      : Characteristic.TargetHeatingCoolingState.AUTO
...
HueSensor.prototype.checkOff = function (off) {
  if (this.obj.config.off !== off) {
    this.log.debug(
      '%s: sensor off changed from %j to %j', this.name,
      this.obj.config.off, off
    )
    this.obj.config.off = off
  }
  const hkTargetHeatingCoolingState = this.obj.config.off
    ? Characteristic.TargetHeatingCoolingState.OFF
    : this.obj.config.boost
      ? Characteristic.TargetHeatingCoolingState.HEAT
      : Characteristic.TargetHeatingCoolingState.AUTO
  if (this.hk.targetHeatingCoolingState !== hkTargetHeatingCoolingState) {
    if (this.hk.targetHeatingCoolingState !== undefined) {
...
HueSensor.prototype.setTargetHeatingCoolingState = function (
  ...
  if (this.model === 'SPZB0001') {
    const boost = this.hk.targetHeatingCoolingState ===
      Characteristic.TargetHeatingCoolingState.HEAT
    const off = this.hk.targetHeatingCoolingState ===
      Characteristic.TargetHeatingCoolingState.OFF
    this.bridge.request('put', this.resource + '/config', {
      boost: boost,
      off: off
    }).then((obj) => {
      this.obj.config.boost = boost
