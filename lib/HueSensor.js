...
function HueSensor (bridge, id, obj) {
  ...
  switch (this.obj.type) {
    ...
    case 'ZHASwitch':
      if (
        ...
        this.createLabel(Characteristic.ServiceLabelNamespace.ARABIC_NUMERALS)
        if (this.subtype === '02') {
          this.createButton(1, 'Left', [SINGLE, LONG])
        } else {
          this.createButton(2, 'Right', [SINGLE, LONG])
        }
        this.type = {
          key: 'buttonevent',
          homekitValue: function (v) { return Math.floor(v / 1000) },
        ...
        this.subtype = this.obj.uniqueid.split('-')[1]
        this.createLabel(Characteristic.ServiceLabelNamespace.ARABIC_NUMERALS)
        const button = parseInt(this.subtype)
        if (button >= 1 && button <= 4) {
          this.createButton(button, this.subtype, [SINGLE, LONG])
        }
        this.type = {
          key: 'buttonevent',
          homekitValue: function (v) { return Math.floor(v / 1000) },
        ...
        this.createLabel(Characteristic.ServiceLabelNamespace.ARABIC_NUMERALS)
        this.createButton(1, 'Button', [SINGLE, DOUBLE])
        this.type = {
          key: 'buttonevent',
          homekitValue: function (v) { return Math.floor(v / 1000) },
          homekitAction: hkZLLSwitchAction
  ...
  if (this.service) {
    if (this.type.Characteristic && this.type.key !== 'open') {
    ...
    if (this.type.key === 'open') {
      this.serviceList.push(this.service)
      this.historyService = new HistoryService('door', this)
      this.hk.timesOpened = 0
      this.hk.offset = moment().unix()
      this.service.addOptionalCharacteristic(Characteristic.LastActivation)
      this.service.addOptionalCharacteristic(Characteristic.TimesOpened)
      this.checkOpen(this.obj.state.open)
      this.service.addCharacteristic(Characteristic.Char118)
      this.service.addCharacteristic(Characteristic.Char119)
      this.historyService.addCharacteristic(Characteristic.ResetTotal)
      this.historyService.getCharacteristic(Characteristic.ResetTotal)
        .on('set', (value, callback) => {
          this.hk.timesOpened = 0
          this.service.updateCharacteristic(
            Characteristic.TimesOpened, this.hk.timesOpened
          )
          callback(null)
        })
    }
...
HueSensor.prototype.checkOpen = function (open) {
  if (this.obj.state.open !== open) {
    this.log.debug(
      '%s: sensor open changed from %s to %s', this.name,
      this.obj.state.open, open
    )
    this.obj.state.open = open
  }
  const hkOpen = this.obj.state.open ? 1 : 0
  if (this.hk.open !== hkOpen) {
    if (this.hk.open !== undefined) {
      this.log.info(
        '%s: set homekit contact from %s to %s', this.name,
        this.hk.open, hkOpen
      )
    }
    this.hk.open = hkOpen
    this.status = this.hk.open
    this.hk.timesOpened += this.hk.open
    const now = moment().unix()
    this.hk.lastActivation = now - this.hk.offset
    this.historyService.addEntry({time: now, status: this.status})
    this.service
      .updateCharacteristic(Characteristic.ContactSensorState, this.hk.open)
      .updateCharacteristic(Characteristic.TimesOpened, this.hk.timesOpened)
      .updateCharacteristic(Characteristic.LastActivation, this.hk.lastActivation)
  }
}
