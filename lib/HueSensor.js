...
function HueSensor (accessory, id, obj) {
  ...
  switch (this.obj.type) {
    ...
    case 'ZHASwitch':
      if (
        ...
        this.obj.manufacturername === 'Signify Netherlands B.V.' &&
        this.obj.modelid === 'RDM001'
      ) {
        // Hue wall switch module
        this.log.debug(
          '%s: %s: devicemode: %s', this.bridge.name, this.resource,
          this.obj.config.devicemode
        )
        const rocker = this.obj.config.devicemode.endsWith('rocker')
        const dual = this.obj.config.devicemode.startsWith('dual')
        this.createLabel(Characteristic.ServiceLabelNamespace.ARABIC_NUMERALS)
        if (rocker) {
        ...
        } else if (this.bridge.platform.config.hueDimmerRepeat) {
          this.repeat = [1, 2]
          this.createButton(1, 'Push Button 1', SINGLE)
          if (dual) {
            this.createButton(2, 'Push Button 2', SINGLE)
          }
        } else {
          this.createButton(1, 'Push Button 1', SINGLE_LONG)
          if (dual) {
            this.createButton(1, 'Push Button 2', SINGLE_LONG)
          }
        }
        this.type = {
          key: 'buttonevent',
        ...
        this.createLabel(Characteristic.ServiceLabelNamespace.ARABIC_NUMERALS)
        if (this.obj.mode === 1) {
          this.createButton(2, 'Turn Right', SINGLE_LONG)
          this.createButton(3, 'Turn Left', SINGLE_LONG)
        } else {
        ...
        this.obj.manufacturername === 'IKEA of Sweden' &&
        this.obj.modelid === 'Remote Control N2'
      ) {
        // Ikea STYRBAR
        this.createLabel(Characteristic.ServiceLabelNamespace.ARABIC_NUMERALS)
        ...
        // Xiaomi Aqara smart wireless switch with gyro
        this.createLabel(Characteristic.ServiceLabelNamespace.ARABIC_NUMERALS)
        this.createButton(1, 'Button', SINGLE_DOUBLE_LONG)
        this.type = {
          key: 'buttonevent',
        ...
        this.obj.manufacturername === 'Lutron' &&
        this.obj.modelid === 'LZL4BWHL01 Remote'
      ) {
        // Lutron Pico, see 102.
        this.createLabel(Characteristic.ServiceLabelNamespace.ARABIC_NUMERALS)
        this.createButton(1, 'On', SINGLE)
        ...
        // Sunricher 8-button remote, see #529.
        this.createLabel(Characteristic.ServiceLabelNamespace.ARABIC_NUMERALS)
        if (this.endpoint === '01') {
        ...
        this.obj.manufacturername === 'MLI' &&
        this.obj.modelid === 'ZBT-Remote-ALL-RGBW'
      ) {
        // Tint remote control by Müller-Licht see deconz-rest-plugin#1209
        this.createLabel(Characteristic.ServiceLabelNamespace.ARABIC_NUMERALS)
        this.createButton(1, 'On/Off', SINGLE)
        ...
        this.obj.manufacturername === 'OSRAM' &&
        this.obj.modelid === 'Lightify Switch Mini'
      ) {
        this.createLabel(Characteristic.ServiceLabelNamespace.ARABIC_NUMERALS)
        ...
        this.obj.manufacturername === '_TZ3000_vp6clf9d' &&
        this.obj.modelid === 'TS0044'
      ) {
        // Tuya 4 Button Switch, see #924.
        this.createLabel(Characteristic.ServiceLabelNamespace.ARABIC_NUMERALS)
        ...
        this.obj.modelid === 'ElkoDimmerRemoteZHA'
      ) {
        // ELKO ESH 316 Endevender RF, see #922.
        this.createLabel(Characteristic.ServiceLabelNamespace.ARABIC_NUMERALS)
        ...
        this.obj.manufacturername === 'lk' &&
        this.obj.modelid === 'ZBT-DIMSwitch-D0001'
      ) {
        // Linkind 1-Key Remote Control, see #949.
        this.createLabel(Characteristic.ServiceLabelNamespace.ARABIC_NUMERALS)
        this.createButton(1, 'Button', SINGLE_LONG)
        this.type = {
          key: 'buttonevent',
          homekitValue: function (v) { return 1000 },
          homekitAction: hkZLLSwitchAction
        }
      } else {
        this.log.warn(
    ...
    case 'CLIPBattery':
      ...
      this.type = {
        ...
        name: 'battery',
        unit: '%',
        homekitValue: function (v) { return toInt(v, 0, 100) }
      }
      break
  ...
  if (this.service) {
    ...
    if (this.obj.config.heatsetpoint !== undefined) {
      ...
      if (this.obj.config.schedule !== undefined) {
        if (this.obj.config.schedule.on !== undefined) {
          this.checkScheduleOn(this.obj.config.schedule.on)
        }
      }
...
HueSensor.prototype.checkMode = function (mode) {
  ...
  let hkTargetHeatingCoolingState
  switch (mode) {
    case 'off':
      hkTargetHeatingCoolingState = Characteristic.TargetHeatingCoolingState.OFF
      break
    case 'heat':
      hkTargetHeatingCoolingState = Characteristic.TargetHeatingCoolingState.HEAT
      break
    case 'auto':
    default:
      hkTargetHeatingCoolingState = Characteristic.TargetHeatingCoolingState.AUTO
      break
  }
...
HueSensor.prototype.setProgramCommand = function (value, callback) {
  ...
  for (let i = 0; i < buffer.length; i++) {
    ...
    switch (opcode) {
      case 0x00: // Begin
        this.log.info('%s:   00 begin', this.name)
        break
      case 0x06: // End
        this.log.info('%s:   06 end', this.name)
        break
      case 0x12: // Offset
        offset = buffer.readInt8(++i) / 10
        this.log.info('%s:   12 offset: %s°C', this.name, offset.toFixed(1))
        break
      case 0x13: // Schedule Enable
        scheduleOn = buffer[++i] === 1
        this.log.info('%s:   13 schudule_on %s', this.name, scheduleOn)
        break
      case 0x1A: // Away transitions
        {
          ...
          this.log.info('%s:   Free%s', this.name, s)
          i += 8
        }
        break
      case 0xF4: // Temperature
        {
          ...
          this.log.info('%s:   F4 temp: %s°C, %s°C, %s°C', this.name, now, low, high)
        }
        break
      case 0xFC: // Time
        {
          ...
          this.log.info('%s:   FC time: %s-%s-%sT%s:%s', this.name, y, m, d, h, n)
        }
        break
      ...
      case 0xFF: // Reset?
        i += 2
        this.log.info('%s:   FF reset', this.name)
        break
      default: // Unknown
        this.log.info(
          '%s:   %s', this.name,
